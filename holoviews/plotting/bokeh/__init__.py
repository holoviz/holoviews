...
Palette.colormaps.update(
    {name: colormap_generator(p[max(p.keys())]) for name, p in all_palettes.items()}
)

Cycle.default_cycles.update(
    {name: p[max(p.keys())] for name, p in all_palettes.items() if max(p.keys()) < 256}
)
...
options.Points = Options(
    "style", color=Cycle(), size=point_size, cmap=dflt_cmap, radius_dimension="min"
)
...
options.Polygons = Options("style", color=Cycle(), line_color="black", cmap=dflt_cmap)
...
options.GridMatrix = Options("plot", shared_xaxis=True, shared_yaxis=True, xaxis=None, yaxis=None)
...
options.Distribution = Options(
    "style", color=Cycle(), line_color="black", fill_alpha=0.5, muted_alpha=0.2
)
options.Violin = Options(
    "style",
    violin_fill_color=Cycle(),
