...
class HeatMapPlot(ColorbarPlot):
    ...
    optimize_gridded = param.Boolean(
        default=True,
        doc="""
    ...
    def _update_hover(self, element):
        ...
        if BOKEH_GE_3_3_0:
            ...
            hover.tooltips = [
                (xdim.pprint_label, "$x{x}"),
                (ydim.pprint_label, "$y{y}"),
                (vdim, "@image"),
            ]
        else:
            xdim, ydim = element.kdims
    ...
    def get_data(self, element, ranges, style):
        ...
        if not element._unique:
            self.param.warning(
                "HeatMap element index is not unique,  ensure you "
        ...
        if self.is_contiguous_gridded:
            ...
            dw = (
                ...
                if x_cat
                else (ranges[x]["data"][1] - ranges[x]["data"][0])
            )
            dh = (
                data["image"][0].shape[0]
                if y_cat
                else (ranges[y]["data"][1] - ranges[y]["data"][0])
            )
            ...
            return (
                {},
                {"x": x, "y": y, "fill_color": {"field": "zvalues", "transform": cmapper}},
                style,
            )
        ...
        style = {
            k: v
            for k, v in style.items()
            if not any(g in k for g in RadialHeatMapPlot._style_groups.values())
        }
        return (
            data,
            {
                "x": x,
                "y": y,
                "fill_color": {"field": "zvalues", "transform": cmapper},
                "height": height,
                "width": width,
            },
            style,
    ...
    def _draw_markers(self, plot, element, marks, axis="x"):
        if marks is None or self.radial:
            return
        self.param.warning(
            "Only radial HeatMaps supports marks, to make the"
...
class RadialHeatMapPlot(CompositeElementPlot, ColorbarPlot):
    ...
    def get_default_mapping(self, z, cmapper):
        ...
        map_seg_label = dict(x="x", y="y", text="text", angle="angle", text_align="center")

        map_ann_label = dict(
            x="x", y="y", text="text", angle="angle", text_align="center", text_baseline="bottom"
        )
