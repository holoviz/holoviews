...
class SizebarMixin(LegendPlot):
    ...
    sizebar_orientation = param.Selector(
        default="horizontal",
    ...
    sizebar_bounds = param.NumericTuple(
        default=None,
        length=2,
        doc="""
...
class PointPlot(SizebarMixin, ColorbarPlot):
    ...
    def get_batched_data(self, element, ranges):
        ...
        for (key, el), zorder in zip(element.data.items(), zorders, strict=None):
            ...
            sdata, smapping = expand_batched_style(
                style, self._batched_style_opts, elmapping, nvals
            )
...
class CurvePlot(ElementPlot):
    ...
    def get_data(self, element, ranges, style):
        ...
        data = {x: element.dimension_values(xidx), y: element.dimension_values(yidx)}
        self._get_hover_data(data, element)
    ...
    def get_batched_data(self, overlay, ranges):
        ...
        for (key, el), zorder in zip(overlay.data.items(), zorders, strict=None):
            ...
            # Apply static styles
            sdata, smapping = expand_batched_style(
                style, self._batched_style_opts, elmapping, nvals=1
            )
            elmapping.update(smapping)
...
class SideHistogramPlot(HistogramPlot):
    ...
    def get_data(self, element, ranges, style):
        ...
        color_dims = [
            d
            for d in self.adjoined.traverse(lambda x: x.handles.get("color_dim"))
            if d is not None
        ]
...
class ErrorPlot(ColorbarPlot):
    ...
    style_opts = (
        [
            p
            for p in line_properties
            if p.split("_")[0] not in ("hover", "selection", "nonselection", "muted")
        ]
...
class BarPlot(BarsMixin, ColorbarPlot, LegendPlot):
    ...
    def _axis_properties(self, axis, key, plot, dimension=None, ax_mapping=None):
        ...
        if (
            not self.multi_level
            and not self.stacked
            and self.current_frame.ndims > 1
            and ((not self.invert_axes and axis == "x") or (self.invert_axes and axis == "y"))
        ):
    ...
    def _element_transform(self, transform, element, ranges):
        if (self.multi_level or self.stacked) and len(element.kdims) > 1:
            return transform.apply(
                element.groupby(element.kdims[1]).collapse(), ranges=ranges, flat=True
            )
    ...
    def get_data(self, element, ranges, style):
        ...
        if "width" in style:
            self.param.warning("BarPlot width option is deprecated use 'bar_width' instead.")
        cmap = style.get("cmap")
        ...
        for k, ds in grouped.items():
            ...
            if grouping == "stacked":
                for sign, slc in [("negative", (None, 0)), ("positive", (0, None))]:
                    ...
                    stack_inds = [
                        stack_order.index(v) if v in stack_order else -1
                        for v in slc_ds[stack_dim.name]
                    ]
                    slc_ds = slc_ds.add_dimension("_stack_order", 0, stack_inds).sort(
                        "_stack_order"
                    )
                    ...
                    if not style_mapping:
                        self._add_color_data(
                            slc_ds, ranges, style, cdim, data, mapping, factors, colors
                        )
            elif grouping == "grouped":
                xs = ds.dimension_values(xdim)
                ys = ds.dimension_values(ydim)
                xoffsets = [
                    (x if dtype_kind(xs) in "SU" else xdim.pprint_value(x), gval) for x in xs
                ]
            ...
            if grouping != "stacked" and not style_mapping:
                self._add_color_data(ds, ranges, style, cdim, data, mapping, factors, colors)
        ...
        if (
            ...
            and dtype_kind(sanitized_data[xname]) not in "uifM"
            and not isdatetime(sanitized_data[xname])
        ):
