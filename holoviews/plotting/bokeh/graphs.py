...
class GraphPlot(GraphMixin, CompositeElementPlot, ColorbarPlot, LegendPlot):
    ...
    inspection_policy = param.Selector(
        default="nodes",
        objects=["edges", "nodes", None],
        doc="""
        Determines policy for inspection of graph components, i.e. whether to highlight
    ...
    style_opts = (
        ["edge_" + p for p in base_properties + fill_properties + line_properties]
        + ["node_" + p for p in base_properties + fill_properties + line_properties]
        + ["node_size", "cmap", "edge_cmap", "node_cmap", "node_radius", "node_marker"]
    )
    ...
    def _hover_opts(self, element):
        if self.inspection_policy == "nodes":
            ...
            kdims = [
                (kd.pprint_label, f"@{{{kd}_values}}") if kd in ("start", "end") else kd
    ...
    def _get_edge_paths(self, element, ranges):
        ...
        if element._edgepaths is not None:
            ...
            arrow_len = (
                np.hypot(y_range[1] - y_range[0], x_range[1] - x_range[0]) * self.arrowhead_length
            )
...
class ChordPlot(ChordMixin, GraphPlot):
    ...
    def _sync_arcs(self):
        ...
        for gtype in ("selection_", "nonselection_", "muted_", "hover_", ""):
            ...
            styles = {
                k.replace("fill", "line"): v for k, v in scatter_props.items() if "fill" in k
            }
...
class TriMeshPlot(GraphPlot):
    ...
    style_opts = (
        ["edge_" + p for p in base_properties + line_properties + fill_properties]
        + ["node_" + p for p in base_properties + fill_properties + line_properties]
