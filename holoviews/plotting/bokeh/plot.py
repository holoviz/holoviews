...
class BokehPlot(DimensionedPlot, CallbackPlot):
    ...
    title = param.String(
        default="{label} {group} {dimensions}",
        doc="""
    ...
    def _get_title_div(self, key, default_fontsize="15pt", width=450):
        ...
        title_tags = self._title_template.format(
            color=color, font=font, fontstyle=fontstyle, fontsize=fontsize, title=title
        )
    ...
    def sync_sources(self):
        ...
        filter_fn = lambda x: (
            x.shared_datasource
            and x.current_frame is not None
            and not isinstance(x.current_frame.data, np.ndarray)
...
class GridPlot(CompositePlot, GenericCompositePlot):
    ...
    def __init__(self, layout, ranges=None, layout_num=1, keys=None, **params):
        ...
        if "axis_offset" in params:
            self.param.warning(
                "GridPlot axis_offset option is deprecated "
        ...
        for i, coord in enumerate(layout.keys(full_grid=True)):
            ...
            if plotting_class is None:
                ...
                collapsed_layout[coord] = (
                    subplot.layout if isinstance(subplot, GenericCompositePlot) else subplot.hmap
                )
    ...
    def _create_subplots(self, layout, positions, layout_dimensions, ranges, num=0):
        ...
        for pos in positions:
            ...
            subplot = plot_type(
                ...
                layout_num=num,
                renderer=self.renderer,
                **dict({"shared_axes": self.shared_axes}, **plotopts),
            )
    ...
    def initialize_plot(self, plots=None, ranges=None):
        ...
        for r in range(self.rows):
            ...
            for c in range(self.cols):
                ...
                if self.tabs:
                    title = subplot.subplots["main"]._format_title(self.keys[-1], dimensions=False)
                    ...
                    if nsubplots == 1:
                        ...
                        children = (
                            [subplots] if nsubplots == 2 else [[subplots[2], None], subplots[:2]]
                        )
                        grid = gridplot(
                            children,
                            merge_tools=False,
