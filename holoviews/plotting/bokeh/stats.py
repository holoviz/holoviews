...
class BoxWhiskerPlot(MultiDistributionMixin, CompositeElementPlot, ColorbarPlot, LegendPlot):
    ...
    style_opts = (
        ["whisker_" + p for p in base_properties + line_properties]
    ...
    def get_data(self, element, ranges, style):
        ...
        whisker_width = style.pop("whisker_width", 0.4) / 2.0
        if "width" in style:
            self.param.warning("BoxWhisker width option is deprecated use 'box_width' instead.")
        if self.invert_axes:
        ...
        data = {
            bar_glyph + "_1": r1_data,
            bar_glyph + "_2": r2_data,
            "segment_1": s1_data,
            "segment_2": s2_data,
            "segment_3": w1_data,
            "segment_4": w2_data,
        ...
        mapping = {
            bar_glyph + "_1": vbar_map,
            bar_glyph + "_2": vbar2_map,
            "segment_1": seg_map,
            "segment_2": seg_map,
            "segment_3": seg_map,
            "segment_4": seg_map,
...
class ViolinPlot(BoxWhiskerPlot):
    ...
    inner = param.Selector(
        objects=["box", "quartiles", "stick", None],
        default="box",
        doc="""
    ...
    style_opts = (
        ...
        + [
            glyph + p
            for p in base_properties + line_properties
            for glyph in ("stats_", "outline_")
        ]
        + [
            f"{glyph}_{p}"
            for p in ("color", "alpha")
    ...
    def _kde_data(self, element, el, key, split_dim, split_cats, **kwargs):
        ...
        if split_dim:
            ...
            fill_ys = [
                [(*key, y) for y in (fy / np.abs(ys).max()) * (self.violin_width / scale)]
                if len(fy)
                else []
                for fy in fill_ys
            ]
    ...
    def get_data(self, element, ranges, style):
        ...
        # Define glyph-data mapping
        if self.invert_axes:
            bar_map = {"y": "x", "left": "bottom", "right": "top", "height": 0.1}
            kde_map = {"xs": "ys", "ys": "xs"}
            if self.inner == "box":
            ...
            bar_map = {"x": "x", "bottom": "bottom", "top": "top", "width": 0.1}
            kde_map = {"xs": "xs", "ys": "ys"}
            if self.inner == "box":
                seg_map = {"x0": "x", "x1": "x", "y0": "y0", "y1": "y1"}
            else:
        ...
        if seg_data:
            data["segment_1"] = {
                k: v if isinstance(v[0], tuple) else np.array(v) for k, v in seg_data.items()
            }
            mapping["segment_1"] = seg_map
        if bar_data:
            data[bar_glyph + "_1"] = {
                k: v if isinstance(v[0], tuple) else np.array(v) for k, v in bar_data.items()
            }
            mapping[bar_glyph + "_1"] = bar_map
        if scatter_data:
            data["scatter_1"] = {
                k: v if isinstance(v[0], tuple) else np.array(v) for k, v in scatter_data.items()
            }
