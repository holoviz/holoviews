...
class RGBPlot(LegendPlot):
    ...
    def _init_glyphs(self, plot, element, ranges, source):
        ...
        if legend is None:
            return
        legend_params = {
            k: v for k, v in self.param.values().items() if k.startswith("legend")
        }
        ...
        self.handles["rgb_color_mapper"] = self._legend_plot.handles[
            "color_color_mapper"
        ]
...
class QuadMeshPlot(ColorbarPlot):
    ...
    def get_data(self, element, ranges, style):
        ...
        irregular = element.interface.irregular(
            element, x
        ) or element.interface.irregular(element, y)
        if irregular:
        ...
        if irregular:
            ...
            if self.invert_axes:
                dims = dims[::-1]
            X, Y = (
                element.interface.coords(element, d, expanded=True, edges=True)
                for d in dims
            )
            X, Y = colormesh(X, Y)
    ...
    def _collect_hover_data(self, element, mask=(), irregular=False):
        ...
        hover_vals = [
            element.dimension_values(hover_dim, flat=False) for hover_dim in hover_dims
        ]
        hover_data = {}
