...
class ElementPlot(BokehPlot, GenericElementPlot):
    ...
    subcoordinate_y = param.ClassSelector(
        default=False,
        class_=(bool, tuple),
        doc="""
    ...
    lod = param.Dict(
        default={"factor": 10, "interval": 300, "threshold": 2000, "timeout": 500},
        doc="""
    ...
    default_tools = param.List(
        default=["save", "pan", "wheel_zoom", "auto_box_zoom", "reset"],
    ...
    toolbar = param.Selector(
        default="right",
        objects=["above", "below", "left", "right", "disable", None],
        doc="""
            ...
            if isinstance(self.hover_tooltips, list):
                ...
                for tooltip in self.hover_tooltips:
                    if isinstance(tooltip, str):
                        ...
                        raise ValueError(
                            "Hover tooltips must be a list with items of strings or tuples."
                        )
    ...
    def _init_tools(self, element, callbacks=None):
        ...
        if self.hover_tooltips:
            no_hover = "hover" not in all_tools and not (
                any(isinstance(tool, tools.HoverTool) for tool in all_tools)
            )
            if no_hover:
        ...
        for tool in all_tools:
            ...
            if tool in ["vline", "hline"]:
                ...
                tool = tools.HoverTool(tooltips=tooltips, tags=["hv_created"], **tool_opts)
            elif (
                BOKEH_GE_3_2_0
                and isinstance(tool, str)
                and tool.endswith(("wheel_zoom", "zoom_in", "zoom_out"))
            ):
                zoom_kwargs = {}
                tags = ["hv_created"]
    ...
    def _update_hover(self, element):
        ...
        if "hv_created" in tool.tags:
            ...
            new_hover = [
                t for t in plot_opts.kwargs.get("tools", []) if isinstance(t, tools.HoverTool)
            ]
            if new_hover:
    ...
    def _subcoord_overlaid(self):
        ...
        self._subcoord_standalone_ = (isinstance(self, OverlayPlot) and self.subcoordinate_y) or (
    ...
    def _axis_props(
        ...
        *,
        ...
        el = element.traverse(
            lambda x: x,
            [lambda el: isinstance(el, Element) and not isinstance(el, (Annotation, Tiles))],
        )
        ...
        if (
            pos == 1
            and "subcoordinate_y" in range_tags_extras
            and dim
            and dim.range != (None, None)
        ):
            dims = [dim]
            v0, v1 = dim.range
            axis_label = str(dim)
            ...
            if pos == 1 and self._subcoord_overlaid:
                if isinstance(self.subcoordinate_y, bool):
                    if self.ylim and all(np.isfinite(val) for val in self.ylim):
                        ...
                        v0, v1 = (
                            0 - offset,
                            sum(self.traverse(lambda p: p.subcoordinate_y)) - 2 + offset,
                        )
                else:
                    v0, v1 = 0, 1
            else:
    ...
    def _init_plot(self, key, element, plots, ranges=None):
        ...
        axis_specs = {"x": {}, "y": {}}
        axis_specs["x"]["x"] = (
            *self._axis_props(plots, subplots, element, ranges, pos=0),
            self.xaxis,
            {},
        )
        if self.multi_y and subplots:
            if not BOKEH_GE_3_2_0:
                self.param.warning(
                    "Independent axis zooming for multi_y=True only supported for Bokeh >=3.2"
                )
        ...
        for axis, axis_spec in axis_specs.items():
            for axis_dim, (
                ...
                if f"{axis}_range" in properties:
                    properties[f"extra_{axis}_ranges"] = extra_ranges = properties.get(
                        f"extra_{axis}_ranges", {}
                    )
                    extra_ranges[axis_dim] = axis_range
                    if not self.subcoordinate_y:
                        properties[f"extra_{axis}_scales"] = extra_scales = properties.get(
                            f"extra_{axis}_scales", {}
                        )
    ...
    def _axis_properties(self, axis, key, plot, dimension=None, ax_mapping=None):
        ...
        if (axis == "x" and self.xaxis in ["bottom-bare", "top-bare", "bare"]) or (
            ...
            if not (self._subcoord_overlaid and axis == "y"):
                ...
                for el, (sp_key, sp) in zip(
                    ...
                    ycenter = (
                        idx
                        if isinstance(sp.subcoordinate_y, bool)
                        else 0.5 * sum(sp.subcoordinate_y)
                    )
                    idx += 1
        ...
        if self.geographic and isinstance(self.projection, str) and self.projection == "mercator":
            ...
            msize = self._fontsize(f"minor_{axis}ticks", common=False).get("fontsize")
    ...
    def _update_grid(self, plot):
        ...
        xopts = {
            k.replace("grid_", "") if any(r in k for r in replace) else k: v
    ...
    def _update_ranges(self, element, ranges):
        ...
        for axis_dim, extra_y_range in self.handles[f"extra_{multi_dim}_ranges"].items():
            ...
            extra_scale = self.handles[f"extra_{multi_dim}_scales"][
                axis_dim
            ]  # Assumes scales and ranges zip
            log = isinstance(extra_scale, LogScale)
            range_update = (
                not (self.model_changed(extra_y_range) or self.model_changed(plot))
                and self.framewise
            )
            ...
            self._update_range(
                ...
                self._shared.get(extra_y_range.name, False),
                log,
                streaming,
            )
    ...
    def _update_main_ranges(self, element, x_range, y_range, ranges, subcoord=False):
        ...
        streaming = self.streaming and any(
            stream._triggering and stream.following for stream in self.streaming
        )
        xupdate = not subcoord and (
            (
                not (self.model_changed(x_range) or self.model_changed(plot))
        ...
        timedelta = BOKEH_GE_3_8_0 and (
            isinstance(xaxis, TimedeltaAxis) or isinstance(yaxis, TimedeltaAxis)
        )
        ...
        if data_aspect and (categorical or datetime or timedelta):
            ...
            if (
                self.drawn
                or (fixed_width and fixed_height)
                or (constrained_width or constrained_height)
            ):
                ...
                if (
                    np.allclose(desired_xspan, xspan, rtol=0.05)
                    and np.allclose(desired_yspan, yspan, rtol=0.05)
                ) or not (util.isfinite(xspan) and util.isfinite(yspan)):
                    pass
        ...
        if self.subcoordinate_y and yupdate and not subcoord:
            ...
            self._update_range(
                ...
                t,
                yfactors,
                self._get_tag(y_range, "invert_yaxis"),
                self._shared["y-main-range"],
                self.logy,
                streaming,
            )
    ...
    def _apply_transforms(self, element, data, ranges, style, group=None):
        ...
        for k, v in dict(style).items():
            ...
            if v.dimension in self.overlay_dims:
                ds = Dataset(
                    {d.name: v for d, v in self.overlay_dims.items()}, list(self.overlay_dims)
                )
            ...
            if (
                not util.isscalar(val)
                and len(util.unique_array(val)) == 1
                and (
            ...
            if not util.isscalar(val):
                if k in self._nonvectorized_styles:
                    ...
                    raise ValueError(
                        f'Mapping a dimension to the "{k}" '
            ...
            if (
                ...
                and (numeric or not validate("color", val) or isinstance(colormap, dict))
            ):
                kwargs = {}
        ...
        for style, val in new_style.copy().items():  # noqa: PLR1704
            for s in ("alpha", "color"):
                if prefix + s != style or style not in data or validate(s, val, True):
                    continue
                supports_fill = any(
                    o.startswith(prefix + "fill")
                    and (prefix != "edge_" or getattr(self, "filled", True))
                ...
                for pprefix in [p + "_" for p in property_prefixes] + [""]:
                    ...
                    if (
                        pprefix in ("nonselection_", "muted_") and s == "alpha"
                    ...
                    if (
                        fill_style is None or (validate(s, fill_style, True) and not hover)
                    ) and supports_fill:
    ...
    def _filter_properties(self, properties, glyph_type, allowed):
        ...
        for gtype in (glyph_type, "") if glyph_type else ("",):
            ...
            props = {k[len(gtype) :]: v for k, v in glyph_props.items() if k.startswith(gtype)}
            if self.batched:
        ...
        for glyph_type in ("", "selection_", "nonselection_", "hover_", "muted_"):
            if renderer:
                ...
                if glyph == "auto":
                    ...
                    glyph = type(base_glyph)(
                        **{k: v for k, v in props.items() if not prop_is_none(v)}
                    )
            ...
            for spec in dataspecs:
                ...
                if (data is None) or (
                    new_field not in data or new_field in source.data or new_field == old_field
                ):
                    continue
    ...
    def _postprocess_hover(self, renderer, source):
        ...
        if not isinstance(hover.tooltips, (str, Div)) and "hv_created" in hover.tags:
            for k, values in source.data.items():
                key = f"@{{{k}}}"
                if (len(values) and isinstance(values[0], util.datetime_types)) or (
                    len(values)
                    and isinstance(values[0], np.ndarray)
                    and dtype_kind(values[0]) == "M"
                ):
                    hover.tooltips = [
                        (l, f + "{%F %T}" if f == key else f) for l, f in hover.tooltips
                    ]
    ...
    def _apply_hard_bounds(self, element, ranges):
        ...
        min_extent_x, min_extent_y, max_extent_x, max_extent_y = map(
            validate_bound,
            self.get_extents(element, ranges, range_type="combined", lims_as_soft_ranges=True),
        )
        ...
        def set_bounds(axis, min_extent, max_extent):
            ...
            try:
                self.handles[axis].bounds = (
                    None if min_extent == max_extent else (min_extent, max_extent)
                )
            except ValueError:
    ...
    def update_frame(self, key, ranges=None, plot=None, element=None):
        ...
        if (
            ...
            or element is None
            or (not self.dynamic and self.static)
            or (
                self.streaming
                and self.streaming[0].data is self.current_frame.data
                and not self.streaming[0]._triggering
            )
        ):
            return
    ...
    def model_changed(self, model):
        ...
        stream_metadata = [
            stream._metadata for cb in callbacks for stream in cb.streams if stream._metadata
        ]
        return any(
            md["id"] == model.ref["id"] for models in stream_metadata for md in models.values()
        )
    ...
    def framewise(self):
        ...
        current_frames = [
            el
            for f in self.traverse(lambda x: x.current_frame)
            for el in (f.traverse(lambda x: x, [Element]) if f else [])
        ]
    ...
    def _draw_scalebar(self, *, plot, renderer):
        ...
        if plot.toolbar and self.scalebar_tool:
            ...
            if existing_tool:
                ...
                active_kwargs = (
                    {"active_callback": "auto", "active": True} if BOKEH_GE_3_7_0 else {}
                )
...
class CompositeElementPlot(ElementPlot):
    ...
    def _init_glyphs(self, plot, element, ranges, source, data=None, mapping=None, style=None):
        ...
        for key in keys:
            ...
            with abbreviated_exception():
                group_style = self._apply_transforms(
                    element, ds_data, ranges, group_style, style_group
                )
            ...
            properties = self._glyph_properties(
            ...
            with abbreviated_exception():
                self._update_glyph(
                    renderer, properties, mapping.get(key, {}), glyph, source, source.data
                )
    ...
    def _update_glyphs(self, element, ranges, style):
        ...
        for key in keys:
            ...
            if glyph:
                ...
                with abbreviated_exception():
                    group_style = self._apply_transforms(
                        element, gdata, ranges, group_style, style_group
                    )
                properties = self._glyph_properties(
                    plot, element, source, ranges, group_style, style_group
                )
...
class ColorbarPlot(ElementPlot):
    ...
    colorbar_specs = {
        "right": {"pos": "right", "opts": {"location": (0, 0)}},
        "left": {"pos": "left", "opts": {"location": (0, 0)}},
        "bottom": {"pos": "below", "opts": {"location": (0, 0), "orientation": "horizontal"}},
        ...
        "bottom_left": {
            "pos": "center",
            "opts": {"location": "bottom_left", "orientation": "horizontal"},
        },
        "bottom_right": {
            "pos": "center",
            "opts": {"location": "bottom_right", "orientation": "horizontal"},
        },
    }

    color_levels = param.ClassSelector(
        default=None,
        class_=(int, list, range),
        doc="""
    ...
    clim_percentile = param.ClassSelector(
        default=False,
        class_=(int, float, bool),
        doc="""
    ...
    _colorbar_defaults = dict(
        bar_line_color="black", label_standoff=8, major_tick_line_color="black"
    )
    ...
    def _draw_colorbar(self, plot, color_mapper, prefix=""):
        ...
        opts = dict(
            cbar_opts["opts"], color_mapper=color_mapper, ticker=ticker, **self._colorbar_defaults
        )
    ...
    def _get_colormapper(
        ...
        if self.adjoined:
            cmappers = self.adjoined.traverse(
                lambda x: (
                    x.handles.get(prefix + "color_dim"),
                    x.handles.get(name),
                    [v for v in x.handles.values() if isinstance(v, ColorMapper)],
                )
            )
        ...
        if eldim:
            ...
            if all(util.isfinite(cl) for cl in self.clim):
                ...
                if (
                    util.is_int(low, int_like=True)
                    and util.is_int(high, int_like=True)
                    and util.is_int(dlow)
                    and util.is_int(dhigh)
                ):
...
class OverlayPlot(GenericOverlayPlot, LegendPlot):
    ...
    style_opts = (
        legend_dimensions
        + ["border_" + p for p in line_properties]
        + text_properties
    ...
    def _process_legend(self, overlay):
        ...
        legend_plots = any(
            p is not None
            for p in subplots
            if isinstance(p, LegendPlot) and not isinstance(p, OverlayPlot)
        )
        ...
        legend_labels = {
            ...
            if isinstance(property_to_dict(i.label), dict)
            else i.label: i
        ...
        for item in legend.items:
            ...
            label = (
                tuple(sorted(item_label.items())) if isinstance(item_label, dict) else item_label
            )
            ...
            if label in legend_labels:
                ...
                prev_item.renderers[:] = list(
                    util.unique_iterator(prev_item.renderers + item.renderers)
                )
            else:
        ...
        for item in legend_items:
            ...
            if (
                item in filtered
                or not item.renderers
                or not any(r.visible or "hv_legend" in r.tags for r in item.renderers)
            ):
                continue
            item_label = property_to_dict(item.label)
            ...
            if el is not None:
                ...
                for tool in el_tools:
                    ...
                    if isinstance(tool, tools.HoverTool):
                        ...
                        self.subcoordinate_y
                        and isinstance(tool, _zoom_types)
                        and "hv_created" in tool.tags
                        and len(tool.tags) == 2
                    ):
    ...
    def _merge_tools(self, subplot):
        ...
        if self.batched and "hover" in subplot.handles:
            ...
            if hover.tooltips and isinstance(hover.tooltips, bokeh.models.dom.Div):
                ...
                tooltips = tuple(
                    (name, spec.replace("{%F %T}", "")) for name, spec in hover.tooltips
                )
            else:
                tooltips = ()
    ...
    def initialize_plot(self, ranges=None, plot=None, plots=None):
        ...
        if self.subcoordinate_y:
            labels = self.hmap.last.traverse(
                ...
                [
                    lambda el: (
                        ...
                        and el.opts.get("plot").kwargs.get("subcoordinate_y", False)
                    )
                ],
            )
            if isinstance(self.hmap.last, NdOverlay):
                ...
                raise ValueError(
                    "Elements wrapped in a subcoordinate_y overlay must all have a unique label."
                )
        key = util.wrap_tuple(self.hmap.last_key)
    ...
    def update_frame(self, key, ranges=None, element=None):
        ...
        for k, subplot in self.subplots.items():
            ...
            if isinstance(self.hmap, DynamicMap) and element:
                ...
                if (
                    triggering
                    and all(s not in triggering for s in subplot.streams)
                    and subplot not in self.dynamic_subplots
                ):
                    continue
