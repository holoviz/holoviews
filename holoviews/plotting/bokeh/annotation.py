...
class _SyntheticAnnotationPlot(ColorbarPlot):
    ...
    def initialize_plot(self, ranges=None, plot=None, plots=None, source=None):
        ...
        if self.overlaid and set(itertools.chain.from_iterable(ranges)) - {
            "HSpans",
            "VSpans",
            "VLines",
            "HLines",
...
class TextPlot(ElementPlot, AnnotationPlot):
    style_opts = (
        text_properties + background_properties + border_properties + ["color", "angle", "visible"]
    )
...
class BoxAnnotationPlot(ElementPlot, AnnotationPlot):
    ...
    def get_data(self, element, ranges, style):
        data = {}
        mapping = dict.fromkeys(("left", "right", "bottom", "top"))
...
class ArrowPlot(CompositeElementPlot, AnnotationPlot):
    style_opts = [
        f"arrow_{p}" for p in line_properties + fill_properties + ["size"]
    ] + text_properties
    ...
    def get_data(self, element, ranges, style):
        ...
        return (
            {"text_1": label_data, "arrow_1": arrow_data},
            {"arrow_1": arrow_mapping, "text_1": label_mapping},
            style,
        )
...
class DivPlot(BokehPlot, GenericElementPlot, AnnotationPlot):
    ...
    def initialize_plot(self, ranges=None, plot=None, plots=None, source=None):
        ...
        div = HTML(
            text=escape(data), width=self.width, height=self.height, sizing_mode=self.sizing_mode
        )
