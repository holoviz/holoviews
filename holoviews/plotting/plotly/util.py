...
def _scale_translate(fig, scale_x, scale_y, translate_x, translate_y):
    ...
    if title:
        ...
        min_fontsize = 12
        titlefont["size"] = round(min_fontsize + (title_fontsize - min_fontsize) * scale_x)
        ...
        for _, axis in axis_pairs[1:]:
            ...
            if "range" in axis and "range" in linked_axis:
                linked_axis["range"] = [
                    ...
                    for v in max_range([axis["range"], linked_axis["range"]])
                ]
    ...
    if isinstance(fig_dict, dict):
        if fig_dict.keys() == {"dtype", "bdata"}:
            ...
            return np.frombuffer(
                base64.b64decode(fig_dict["bdata"]), dtype=fig_dict["dtype"]
            ).reshape(shape)
