...
class HeatMapPlot(HeatMapMixin, QuadMeshPlot):
    ...
    def _annotate_plot(self, ax, annotations):
        ...
        for plot_coord, text in annotations.items():
            handles[plot_coord] = ax.annotate(
                text,
                xy=plot_coord,
                xycoords="data",
    ...
    def init_artists(self, ax, plot_args, plot_kwargs):
        ...
        plot_kwargs = {k: v for k, v in plot_kwargs.items() if not any(p in k for p in prefixes)}
        ...
        self._draw_markers(ax, self.current_frame, self.xmarks, plot_args[0], xfactors, axis="x")
        self._draw_markers(ax, self.current_frame, self.ymarks, plot_args[1], yfactors, axis="y")
...
class RadialHeatMapPlot(ColorbarPlot):
    ...
    def get_data(self, element, ranges, style):
        ...
        segment_ticks = self._map_order_to_ticks(start_angle, end_angle, xvals, True)

        radius_max = 0.5
        radius_min = radius_max * self.radius_inner
        bins_annular = np.linspace(radius_min, radius_max, len(yvals) + 1)
        radius_ticks = self._map_order_to_ticks(radius_min, radius_max, yvals)
    ...
    def init_artists(self, ax, plot_args, plot_kwargs):
        ...
        if paths:
            ...
            yseparators = PatchCollection(
                paths, facecolor="none", transform=ax.transAxes, **ymark_opts
            )
