...
class ViolinPlot(BoxPlot):
    ...
    def init_artists(self, ax, plot_args, plot_kwargs):
        ...
        artists = ax.violinplot(
            *plot_args, bw_method=bw_method, showmedians=showmedians, **plot_kwargs
        )
        if self.inner == "box":
            if MPL_GE_3_10_0:
    ...
    def get_data(self, element, ranges, style):
        ...
        style = {k: v for k, v in new_style.items() if k not in ["zorder", "label"]}
        if MPL_GE_3_10_0:
            style["orientation"] = "horizontal" if self.invert_axes else "vertical"
        else:
            style["vert"] = not self.invert_axes
