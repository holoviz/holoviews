...
class ErrorPlot(ColorbarPlot):
    ...
    def get_data(self, element, ranges, style):
        ...
        if isinstance(c, np.ndarray):
            with abbreviated_exception():
                raise ValueError(
                    "Mapping a continuous or categorical "
...
class HistogramPlot(ColorbarPlot):
    ...
    def initialize_plot(self, ranges=None):
        ...
        with abbreviated_exception():
            ...
            if "vmin" in style:
                raise ValueError(
                    "Mapping a continuous dimension to a "
...
class SideHistogramPlot(AdjoinedPlot, HistogramPlot):
    ...
    def _update_plot(self, key, element, bars, lims, ranges):
        ...
        if offset and ("offset_line" not in self.handles):
            self.handles["offset_line"] = self.offset_linefn(offset, linewidth=1.0, color="k")
        elif offset:
...
class PointPlot(ChartPlot, ColorbarPlot, LegendPlot):
    ...
    _nonvectorized_styles = ["alpha", "marker", "cmap", "vmin", "vmax", "norm", "visible"]
...
class BarPlot(BarsMixin, ColorbarPlot, LegendPlot):
    ...
    legend_specs = dict(
        ...
        bottom=dict(
            bbox_to_anchor=(0.0, -0.4, 1.0, 0.102), ncol=3, loc=2, mode="expand", borderaxespad=0.1
        ),
    )
    ...
    def _create_bars(self, axis, element, ranges, style):
        ...
        if is_dt or dtype_kind(xvals) not in "OU" and not (cdim or len(element.kdims) > 1):
            ...
            width = (1 - self.bar_padding) * (
                date2num(xdiff.astype("timedelta64[us]")) if is_dt else xdiff
            )
