...
class ArrowPlot(AnnotationPlot):
    ...
    def draw_annotation(self, axis, data, opts):
        ...
        arrowprops = dict(
            {"arrowstyle": arrowstyle}, **{k: opts[k] for k in self._arrow_style_opts if k in opts}
        )
        ...
        if "fontsize" in textopts:
            self.param.warning(
                "Arrow fontsize style option is deprecated, use textsize option instead."
            )
...
class SplinePlot(AnnotationPlot):
    ...
    def draw_annotation(self, axis, data, opts):
        ...
        if not len(verts):
            return []
        patch = patches.PathPatch(mpl.path.Path(verts, codes), facecolor="none", **opts)
