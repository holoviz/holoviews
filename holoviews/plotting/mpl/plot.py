...
class MPLPlot(DimensionedPlot):
    ...
    def __init__(self, fig=None, axis=None, **params):
        ...
        self.fig_scale = self.fig_size / 100.0
        if isinstance(self.fig_inches, (tuple, list)):
            self.fig_inches = [None if i is None else i * self.fig_scale for i in self.fig_inches]
        else:
    ...
    def _subplot_label(self, axis):
        ...
        if self.sublabel_format and not self.adjoined and sublabel_num > 0:
            ...
            at = AnchoredText(
                self.sublabel_format.format(**labels),
                loc=3,
...
class GridPlot(CompositePlot):
    ...
    def _create_subplots(self, layout, axis, ranges, create_axes):
        ...
        for coord in layout.keys(full_grid=True):
            ...
            if (
                vtype
                and issubclass(vtype, CompositeOverlay)
                and (c == self.cols - 1 and r == self.rows // 2)
            ):
                kwargs["show_legend"] = self.show_legend
                kwargs["legend_position"] = "right"
            if not isinstance(self.layout, GridMatrix) and not (
                (c == self.cols // 2 and r == 0) or (c == 0 and r == self.rows // 2)
            ):
                kwargs["labelled"] = []
            ...
            if view is not None:
                ...
                collapsed_layout[coord] = (
                    subplot.layout if isinstance(subplot, CompositePlot) else subplot.hmap
                )
...
class AdjointLayoutPlot(MPLPlot, GenericAdjointLayoutPlot):
    ...
    def adjust_positions(self, redraw=True):
        ...
        if right:
            ...
            ax.set_position(
                [
                    bbox.x1 + bbox.width * border_size,
                    bbox.y0,
                    bbox.width * subplot_size,
                    bbox.height,
                ]
            )
...
class LayoutPlot(GenericLayoutPlot, CompositePlot):
    ...
    def _compute_gridspec(self, layout):
        ...
        widths = (
            np.array([r for col in col_widthratios.values() for ratios in col[1] for r in ratios])
            / 4
        )
        ...
        for r, c in self.coords:
            ...
            _, _, projs = self._create_subplots(
                layouts[(r, c)], positions, None, frame_ranges, create=False
            )
    ...
    def _create_subplots(
        self, layout, positions, layout_dimensions, ranges, axes=None, num=1, create=True
    ):
        """Plot all the views contained in the AdjointLayout Object using axes
        appropriate to the layout configuration. All the axes are
        supplied by LayoutPlot - the purpose of the call is to
        ...
        for pos in positions:
            ...
            if pos == "main":
                ...
                sublabel_opts = {k: v for k, v in own_params if "sublabel_" in k}
            elif pos == "right":
