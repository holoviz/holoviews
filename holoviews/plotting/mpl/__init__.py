...
styles = {"default": "./default.mplstyle", "default>1.5": "./default1.5.mplstyle"}

# Define Palettes and cycles from matplotlib colormaps
Palette.colormaps.update(
    {cm: plt.get_cmap(cm) for cm in plt.cm.datad if not ("spectral" in cm or "Vega" in cm)}
)
...
GridPlot = PlotSelector(
    grid_selector, plot_classes=[("GridPlot", BasicGridPlot), ("RasterGridPlot", RasterGridPlot)]
)
...
Store.register(
    {
        ...
        Raster: RasterPlot,
        HeatMap: PlotSelector(
            HeatMapPlot.is_radial, {True: RadialHeatMapPlot, False: HeatMapPlot}, True
        ),
        Image: RasterPlot,
        ImageStack: RGBPlot,
        RGB: RGBPlot,
...
MPLPlot.sideplots.update(
    {
        Histogram: SideHistogramPlot,
        Area: SideAreaPlot,
        GridSpace: GridPlot,
...
fire_r_cmap = LinearSegmentedColormap.from_list(
    "fire_r", list(reversed(fire_colors)), N=len(fire_colors)
)
register_cmap("fire", cmap=fire_cmap)
register_cmap("fire_r", cmap=fire_r_cmap)

register_cmap(
    "kbc_r", cmap=LinearSegmentedColormap.from_list("kbc_r", list(reversed(kbc)), N=len(kbc))
)
...
options.Waterfall = Options("style", edgecolor="k", color=Cycle())
...
options.HeatMap = Options(
    "style",
    cmap=config.default_heatmap_cmap,
    edgecolors="white",
    annular_edgecolors="white",
    annular_linewidth=0.5,
    xmarks_edgecolor="white",
    xmarks_linewidth=3,
    ymarks_edgecolor="white",
    ymarks_linewidth=3,
    linewidths=0,
)
...
options.GridMatrix = Options(
    "plot", fig_size=160, shared_xaxis=True, shared_yaxis=True, xaxis=None, yaxis=None
)
...
options.Polygons = Options("style", facecolor=Cycle(), edgecolor="black", cmap=dflt_cmap)
...
options.Graph = Options(
    "style", node_edgecolors="black", node_facecolors=Cycle(), edge_color="black", node_size=15
)
options.TriMesh = Options(
    "style",
    node_edgecolors="black",
    node_facecolors="white",
    edge_color="black",
    node_size=5,
    edge_linewidth=1,
    cmap=dflt_cmap,
...
options.Chord = Options(
    "style",
    node_edgecolors="black",
    node_facecolors=Cycle(),
    edge_color="black",
    node_size=10,
    edge_linewidth=0.5,
)
options.Chord = Options("plot", xaxis=None, yaxis=None)
options.Nodes = Options("style", edgecolors="black", facecolors=Cycle(), marker="o", s=20**2)
options.EdgePaths = Options("style", color="black")
options.Sankey = Options(
    "plot", xaxis=None, yaxis=None, fig_size=400, aspect=1.6, show_frame=False
)
options.Sankey = Options(
    "style", edge_color="grey", node_edgecolors="black", edge_alpha=0.6, node_size=6
)
