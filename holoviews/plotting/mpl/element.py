...
class ElementPlot(GenericElementPlot, MPLPlot):
    ...
    def _update_grid(self, axis):
        ...
        both = {k.removeprefix("grid_"): v for k, v in style_items if not k.startswith(("x", "y"))}

        # Merge options - both options first, then axis-specific
        xopts = {
            **both,
            **{
                k.removeprefix("xgrid_").removeprefix("x_"): v
        ...
        yopts = {
            **both,
            **{
                k.removeprefix("ygrid_").removeprefix("y_"): v
                for k, v in style_items
                if k.startswith("y")
            },
        }
    ...
    def get_aspect(self, xspan, yspan):
        ...
        if isinstance(self.aspect, (int, float)):
            ...
            if isinstance(xspan, util.datetime_types) ^ isinstance(
                yspan, util.datetime_types
    ...
    def _set_axis_limits(self, axis, view, subplots, ranges):
        ...
        coords = [
            coord if isinstance(coord, np.datetime64) or np.isreal(coord) else np.nan
            for coord in extents
        ]
        coords = [
            date2num(util.dt64_to_dt(c)) if isinstance(c, np.datetime64) else c for c in coords
        ]
    ...
    def _set_axis_position(self, axes, axis, option):
        ...
        if not self.overlaid and not self.show_frame and self.projection != "polar":
            pos = (
                positions[1]
                if (option and (option == "bare" or positions[0] in option))
                else positions[0]
            )
            axes.spines[pos].set_visible(False)
    ...
    def update_frame(self, key, ranges=None, element=None):
        ...
        self._finalize_axis(
            key, element=element, ranges=ranges, **(axis_kwargs if axis_kwargs else {})
        )
    ...
    def render_artists(self, element, ranges, style, ax):
        ...
        if legend and "artist" in handles and hasattr(handles["artist"], "legend_elements"):
            ...
            leg = ax.legend(
                legend_handles, legend["factors"], title=legend["title"], **self._legend_opts
            )
            ax.add_artist(leg)
    ...
    def init_artists(self, ax, plot_args, plot_kwargs):
        ...
        return {"artist": artist[0] if isinstance(artist, list) and len(artist) == 1 else artist}
    ...
    def _apply_transforms(self, element, ranges, style):
        ...
        for k, v in style.items():
            ...
            if v.dimension in self.overlay_dims:
                ...
                val = np.concatenate(
                    [v.apply(el, ranges=ranges, flat=True) for el in element.split()]
                )
            elif "node" in k:
            ...
            if (
                ...
                and ("color" not in k or validate("color", val))
            ):
                val = val[0]
        ...
        for k, val in list(new_style.items()):
            ...
            supports_fill = (prefix != "edge" or getattr(self, "filled", True)) and any(
                o.startswith(prefix + "face") for o in self.style_opts
            )
...
class ColorbarPlot(ElementPlot):
    ...
    def _draw_colorbar(self, element=None, dimension=None, redraw=True):
        ...
        for i, (_artist, cax, _spec, _label) in enumerate(ax_colorbars):
            scaled_w = w * width
            cax.set_position(
                [l + w + padding + (scaled_w + padding + w * 0.15) * i, b, scaled_w, h]
            )
    ...
    def _norm_kwargs(self, element, ranges, opts, vdim, values=None, prefix=""):
        ...
        if values is None:
            if isinstance(vdim, dim):
                ...
                expanded = not (
                    ...
                    and element.interface.multi
                    and (
                        getattr(element, "level", None) is not None
                        or element.interface.isunique(element, vdim.name, True)
                    )
                )
        ...
        if self.cnorm == "eq_hist":
            opts[prefix + "norm"] = EqHistNormalize(
                vmin=clim[0], vmax=clim[1], rescale_discrete_levels=self.rescale_discrete_levels
            )
        ...
        if not isinstance(cmap, mpl_colors.Colormap):
            if isinstance(cmap, dict):
                ...
                if range_key in ranges and "factors" in ranges[range_key]:
                    ...
                    factors = util.unique_array(values)
                palette = [
                    cmap.get(f, colors.get("NaN", {"color": self._default_nan})["color"])
                    for f in factors
                ]
            else:
                palette = process_cmap(cmap, ncolors, categorical=categorical)
                if isinstance(self.color_levels, list):
                    palette, (vmin, vmax) = color_intervals(
                        palette, self.color_levels, clip=(vmin, vmax)
                    )
...
class LegendPlot(ElementPlot):
    ...
    legend_specs = {
        "inner": {},
        "best": {},
        "left": dict(bbox_to_anchor=(-0.15, 1), loc=1),
        "right": dict(bbox_to_anchor=(1.05, 1), loc=2),
        "top": dict(
            bbox_to_anchor=(0.0, 1.02, 1.0, 0.102), ncol=3, loc=3, mode="expand", borderaxespad=0.0
        ),
        "bottom": dict(
            ncol=3,
            mode="expand",
    ...
    def _adjust_legend(self, overlay, axis):
        ...
        for key, subplot in self.subplots.items():
            ...
            handle = subplot.traverse(
                lambda p: p.handles["artist"], [lambda p: "artist" in p.handles]
            )
        ...
        if (not len(set(data.values())) > 0) or not self.show_legend:
            ...
            leg = axis.legend(
                list(data.keys()), list(data.values()), title=title, **self._legend_opts
            )
