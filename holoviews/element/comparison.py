...
def __getattr__(attr):
    if attr == "assert_element_equal":
        ...
        deprecated(
            "1.25.0",
            old="holoviews.element.comparison.assert_element_equal",
            new="holoviews.testing.assert_element_equal",
        )
...
class Comparison(ComparisonInterface):
    ...
    def bounds_check(cls, el1, el2, msg=None):
        ...
        try:
            ...
            raise cls.failureException(
                f"BoundingBoxes are mismatched: {el1.bounds.lbrt()} != {el2.bounds.lbrt()}."
            ) from e
    ...
    def compare_dimensions(cls, dim1, dim2, msg=None):
        ...
        if dim1.label != dim2.label:
            raise cls.failureException(
                f"Dimension labels mismatched: {dim1.label} != {dim2.label}"
            )
        ...
        if set(dim1_params.keys()) != set(dim2_params.keys()):
            raise cls.failureException(
                f"Dimension parameter sets mismatched: {set(dim1_params.keys())} != {set(dim2_params.keys())}"
            )
    ...
    def compare_dimensioned(cls, obj1, obj2, msg=None):
        ...
        cls.compare_dimension_lists(obj1.vdims, obj2.vdims, "Value dimension list")
        cls.compare_dimension_lists(obj1.kdims, obj2.kdims, "Key dimension list")
    ...
    def compare_ndmappings(cls, el1, el2, msg="NdMappings"):
        ...
        if set(el1.keys()) != set(el2.keys()):
            ...
            raise cls.failureException(
                f"{msg} have different sets of keys. "
    ...
    def compare_dataset(cls, el1, el2, msg="Dataset"):
        ...
        dimension_data += [
            (d, el1.dimension_values(d, flat=tabular), el2.dimension_values(d, flat=tabular))
            for d in el1.vdims
        ]
        if el1.shape[0] != el2.shape[0]:
            raise AssertionError(
                f"{msg} not of matching length, {el1.shape[0]} vs. {el2.shape[0]}."
            )
