...
_Styles = Stream.define("Styles", colors=[], alpha=1.0)
...
class _base_link_selections(param.ParameterizedFunction):
    ...
    def __call__(self, hvobj, **kwargs):
        ...
        if Store.current_backend not in Store.renderers:
            raise RuntimeError(
                "Cannot perform link_selections operation "
    ...
    def _selection_transform(self, hvobj, operations=(), origin=None):
        ...
        if isinstance(hvobj, DynamicMap):
            ...
            if len(callback.inputs) > 1:
                return Overlay(
                    [
                        self._selection_transform(
                            el, operations, hvobj if origin is None else origin
                        )
            ...
            if issubclass(hvobj.type, Element):
                ...
                return chart.selection_display(hvobj).build_selection(
                    self._selection_streams,
                    hvobj,
                    operations,
                    self._selection_expr_streams.get(hvobj, None),
                    cache=self._cache,
                )
            elif (
                issubclass(hvobj.type, Overlay)
                and getattr(hvobj.callback, "name", None) == "dynamic_mul"
            ):
                return Overlay(
                    [
            ...
            if getattr(chart, "selection_display", None):
                ...
                return chart.selection_display(element).build_selection(
                    ...
                    element,
                    operations,
                    self._selection_expr_streams.get(element, None),
                    cache=self._cache,
                )
            return hvobj
...
class link_selections(_base_link_selections):
    ...
    selection_mode = param.Selector(
        objects=["overwrite", "intersect", "union", "inverse"],
        default="overwrite",
        doc="""
    ...
    def _build_selection_streams(cls, inst):
        ...
        style_stream = _Styles(
            colors=[inst.unselected_color, inst.selected_color], alpha=inst.unselected_alpha
        )
        ...
        if isinstance(selection_expr, dim):
            ...
            try:
                if dataset.interface.gridded:
                    if mask is None:
                        mask = selection_expr.apply(
                            dataset, expanded=True, flat=False, strict=False
                        )
                    selection = dataset.clone(dataset.interface.mask(dataset, ~mask))
                elif dataset.interface.multi:
                    if mask is None:
                        mask = selection_expr.apply(
                            dataset, expanded=False, flat=False, strict=False
                        )
