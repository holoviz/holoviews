...
class ParamFilter(param.ParameterizedFunction):
    ...
    def __call__(self, obj, filter_fn=None):
        ...
        class_proxy = type(
            name,
            (param.Parameterized,),
            {k: v for k, v in obj.param.objects("existing").items() if filter_fn(k, v)},
        )
        ...
        if isinstance(obj, type):
            ...
            filtered = {
                ...
                if (k in obj_proxy.param) and not obj_proxy.param.objects("existing")[k].constant
            }
...
class InfoPrinter:
    ...
    def highlight(cls, pattern, string):
        if pattern is None:
            return string
        return re.sub(pattern, "\033[43;1;30m\\g<0>\x1b[0m", string, flags=re.IGNORECASE)

    @classmethod
    def info(
        cls, obj, ansi=False, backend="matplotlib", visualization=True, pattern=None, elements=None
    ):
    ...
    def options_info(cls, plot_class, ansi=False, pattern=None):
        ...
        lines = [
            cls.heading("Style Options", ansi=ansi, char="-"),
            "",
            style_msg,
            "",
        ...
        if param_info is not None:
            ...
            lines += [
                f"No {plot_class.__name__!r} parameters found matching specified pattern {pattern!r}."
            ]
        else:
...
class PrettyPrinter(param.Parameterized):
    ...
    @bothmethod
    def recurse(
        cls_or_slf, node, attrpath=None, attrpaths=None, siblings=None, level=0, value_dims=True
    ):
    ...
    def option_info(cls_or_slf, node):
        ...
        for g in Options._option_groups:
            gopts = Store.lookup_options(
                Store.current_backend, node, g, defaults=cls_or_slf.show_defaults
            )
            if gopts:
