...
class HoloMap(Layoutable, UniformNdMapping, Overlayable):
    ...
    def _dynamic_mul(self, dimensions, other, keys):
        ...
        if map_obj:
            return map_obj.clone(
                callback=callback, shared_data=False, kdims=dimensions, streams=streams
            )
        else:
        ...
        if isinstance(other, HoloMap):
            ...
            for dim_keys in super_keys:
                ...
                self_key = tuple(
                    ...
                    for p, k in sorted(
                        [
                            (self.get_dimension_index(dim), v)
                            for dim, v in dim_keys
            ...
            if isinstance(self, DynamicMap):
                ...
                callback._is_overlay = True
                return self.clone(
                    shared_data=False, callback=callback, streams=util.dimensioned_streams(self)
                )
...
class Callable(param.Parameterized):
    ...
    def __init__(self, callable, **params):
        super().__init__(callable=callable, **dict(params, name=util.callable_name(callable)))
        self._memoized = {}
    ...
    def __call__(self, *args, **kwargs):
        ...
        **kwargs
        ...
        memoize = self._stream_memoization and not any(
            s.transient and s._triggering for s in streams
        )
        ...
        if self.argspec.varargs is not None:
            ...
            if ignored:
                self.param.warning(
                    "Ignoring extra positional argument {}".format(
...
class periodic:
    ...
    def __call__(self, period, count=None, param_fn=None, timeout=None, block=True):
        ...
        if self.instance is not None and not self.instance.completed:
            raise RuntimeError(
                "Periodic process already running. "
        ...
        instance = self._periodic_util(period, count, inner, timeout=timeout, block=block)
        instance.start()
...
class DynamicMap(HoloMap):
    ...
    def __init__(self, callback, initial_items=None, streams=None, **params):
        ...
        elif params.get("watch", True) and (
            util.is_param_method(callback, has_deps=True)
        ...
        if invalid:
            msg = (
                "The supplied streams list contains objects that are not Stream instances: {objs}"
            )
    ...
    def clone(self, data=None, shared_data=True, new_type=None, link=True, *args, **overrides):
        ...
        **overrides
        ...
        clone = super(UniformNdMapping, self).clone(
            callback, shared_data, new_type, link, *(data, *args), **overrides
        )
        ...
        if clone.callback is self.callback:
            ...
            with util.disable_constant(clone):
                ...
                clone.callback = clone.callback.clone(
                    inputs=[self], link_inputs=link, operation=op, operation_kwargs={}
                )
        return clone
    ...
    def _cross_product(self, tuple_key, cache, data_slice):
        ...
        if data_slice:
            ...
            dmap = Dynamic(
                self, operation=lambda obj, **dynkwargs: obj[data_slice], streams=self.streams
            )
    ...
    def map(self, map_fn, specs=None, clone=True, link_inputs=True):
        ...
        if isinstance(deep_mapped, type(self)):
            ...
            dmap = Dynamic(
                self, operation=apply_map, streams=self.streams, link_inputs=link_inputs
            )
        ...
        for key in keys:
            ...
            def split_overlay_callback(obj, overlay_key=key, overlay_el=el, **kwargs):
                ...
                if match is None:
                    ...
                    raise KeyError(
                        f"{spec} spec not found in {otype}. The split_overlays method "
    ...
    def collate(self, merge_type=None, drop=None, drop_constant=False):
        ...
        for i, (k, v) in enumerate(initialized.last.data.items()):
            ...
            def collation_cb(*args, **kwargs):
                ...
                if len(container.keys()) != len(layout.keys()):
                    raise ValueError(
                        "Collated DynamicMaps must return "
                ...
                if dyn_type_counter != type_counter:
                    raise ValueError(
                        f"The objects in a {layout_type} returned by a "
        ...
        unmapped_streams = [
            repr(stream)
            for stream in self.streams
            if (stream.source is self) and (stream not in remapped_streams) and stream.linked
        ]
        if unmapped_streams:
            raise ValueError(
    ...
    def hist(
        ...
        def dynamic_hist(obj, **dynkwargs):
            if isinstance(obj, (NdOverlay, Overlay)):
                ...
                obj = obj.get(index)
            return obj.hist(
                dimension=dimension, num_bins=num_bins, bin_range=bin_range, adjoin=False, **kwargs
            )
    ...
    def drop_dimension(self, dimensions):
        raise NotImplementedError(
            "Cannot drop dimensions from a DynamicMap, cast to a HoloMap first."
        )

    def add_dimension(self, dimension, dim_pos, dim_val, vdim=False, **kwargs):
        raise NotImplementedError(
            "Cannot add dimensions to a DynamicMap, cast to a HoloMap first."
        )
...
class GridSpace(Layoutable, UniformNdMapping):
    ...
    def last(self):
        ...
        if self.type == HoloMap:
            last_items = [
                (k, v.last if isinstance(v, HoloMap) else v) for (k, v) in self.data.items()
            ]
        else:
