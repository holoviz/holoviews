...
class ImageInterface(GridInterface):
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if not isinstance(data, np.ndarray) or data.ndim not in [2, 3]:
            ...
            raise ValueError(
                "ImageInterface may only be used on elements that subclass SheetCoordinateSystem."
            )
    ...
    def select(cls, dataset, selection_mask=None, **selection):
        ...
        selection = {
            k: slice(*sel) if isinstance(sel, tuple) else sel for k, sel in selection.items()
        }
        coords = tuple(
    ...
    def groupby(cls, dataset, dim_names, container_type, group_type, **kwargs):
        ...
        if len(dimensions) == 1:
            ...
            samples = [
                (i, slice(None)) if didx else (slice(None), i)
