...
class PandasInterface(Interface, PandasAPI):
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if util.is_dataframe(data):
            ...
            if kdims and not (
                len(kdims) == len(index_names)
                and {dimension_name(kd) for kd in kdims} == set(index_names)
            ):
                kdim = dimension_name(kdims[0])
                if (
                    eltype._auto_indexable_1d
                    and ncols == 1
                    and kdim not in [*data.columns, *cls.indexes(data)]
                ):
                    data = data.copy()
            ...
            if isinstance(data, dict) and all(c in data for c in columns):
                ...
                and not all(d in data for d in columns)
            ...
            if isinstance(data, tuple):
                ...
                if any(d.ndim > 1 for d in data):
                    ...
                    raise DataError(
                        f"Data contains fewer columns than the {eltype.__name__} element expects. Expected "
                ...
                raise ValueError(
                    "PandasInterface could not find specified dimensions in the data."
                )
            else:
    ...
    def aggregate(cls, dataset, dimensions, function, **kwargs):
        ...
        if len(dimensions):
            ...
            if function in [np.size]:
                ...
                numeric_cols = [c for c in reindexed.columns if c not in cols]
            else:
