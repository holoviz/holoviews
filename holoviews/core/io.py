...
class Pickler(Exporter):
    ...
    def save(self_or_cls, obj, filename, key=None, info=None, **kwargs):
        ...
        filename = (
            self_or_cls._filename(filename)
        ...
        with zipfile.ZipFile(filename, "w", compression=compression) as f:
            if isinstance(obj, Layout) and not isinstance(obj, Overlay):
                ...
                entries = [
                    f"{group_sanitizer(obj.group, False)}.{label_sanitizer(obj.label, False)}"
                ]
                components = [obj]
...
class Unpickler(Importer):
    ...
    def collect(self_or_cls, files, drop=None, metadata=True):
        ...
        mdata_dims = []
        if metadata:
            fnames = [fname[0] if isinstance(fname, tuple) else fname for fname in files.values()]
            mdata_dims = {kdim for fname in fnames for kdim in self_or_cls.key(fname).keys()}
        file_dims = set(files.dimensions("key", label=True))
        added_dims = set(mdata_dims) - file_dims
        overlap_dims = file_dims & set(mdata_dims)
        kwargs = dict(
            kdims=file_kdims + sorted(added_dims),
        ...
        for key, fname in files.data.items():
            ...
            for odim in overlap_dims:
                ...
                if kval != mdata[odim]:
                    raise KeyError(f"Metadata supplies inconsistent value for dimension {odim}")
...
class FileArchive(Archive):
    ...
    dimension_formatter = param.String(
        "{name}_{range}",
        doc="""
    ...
    object_formatter = param.Callable(
        default=simple_name_generator,
        doc="""
    ...
    filename_formatter = param.String(
        "{dimensions},{obj}",
        doc="""
    ...
    def export(self, timestamp=None, info=None):
        ...
        files = [
            ((self._format(base, info), ext), val) for ((base, ext), val) in self._files.items()
        ]
