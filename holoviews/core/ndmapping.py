...
class MultiDimensionalMapping(Dimensioned):
    ...
    def _item_check(self, dim_vals, data):
        ...
        if not self._check_items:
            ...
            raise TypeError(
                f"{slf} does not accept {data} type, data elements have to be a {data_type}."
            )
        elif not len(dim_vals) == self.ndims:
            raise KeyError(
                f"The data contains keys of length {len(dim_vals)}, but the kdims "
    ...
    def _apply_key_type(self, keys):
        ...
        for dim, key in zip(self.kdims, keys, strict=False):
            ...
            if key_type is None:
                ...
                typed_key += (
                    slice(*[key_type(el) if el is not None else None for el in sl_vals]),
                )
    ...
    def groupby(self, dimensions, container_type=None, group_type=None, **kwargs):
        ...
        **kwargs
        ...
        with item_check(False):
            ...
            return util.ndmapping_groupby(
                self, dimensions, container_type, group_type, sort=sort, **kwargs
            )
    ...
    def dimension_values(self, dimension, expanded=True, flat=True):
        ...
        if dimension in self.dimensions():
            values = [
                el.dimension_values(dimension, expanded, flat)
                for el in self
    ...
    def update(self, other):
        ...
        if isinstance(other, NdMapping):
            ...
            if len(dims) == other.ndims:
                raise KeyError(
                    "Cannot update with NdMapping that has a different set of key dimensions."
                )
            elif dims:
...
class NdMapping(MultiDimensionalMapping):
    ...
    def __getitem__(self, key):
        ...
        if all(
            not (isinstance(el, (slice, set, list, tuple)) or callable(el)) for el in map_slice
        ):
            return self._dataslice(self.data[map_slice], data_slice)
        else:
        ...
        for dim, dim_slice in zip(self.kdims, map_slice, strict=False):
            if isinstance(dim_slice, slice):
                ...
                if dim.values:
                    values = dim.values
                    dim_slice = slice(
                        None if start is None else values.index(start),
...
class UniformNdMapping(NdMapping):
    ...
    def clone(self, data=None, shared_data=True, new_type=None, link=True, *args, **overrides):
        ...
        **overrides
        ...
        with item_check(not shared_data and self._check_items):
            return clone_type(
                data, *args, **{k: v for k, v in settings.items() if k not in pos_args}
            )
    ...
    def _item_check(self, dim_vals, data):
        if not self._check_items:
            ...
            raise AssertionError(
                f"{self.__class__.__name__} must only contain one type of object, not both {type(data).__name__} and {self.type.__name__}."
            )
    ...
    def __mul__(self, other, reverse=False):
        ...
        if isinstance(other, type(self)):
            if self.kdims != other.kdims:
                raise KeyError(
                    f"Can only overlay two {type(self).__name__}s with "
        ...
        overlaid_items = [(k, other * el if reverse else el * other) for k, el in self.items()]
        try:
