    ...
    def __call__(
        ...
        **kwargs : dict, optional
        ...
        if isinstance(self._obj, DynamicMap) and dynamic == False:
            ...
            if not all(samples):
                raise ValueError(
                    "Applying a function to a DynamicMap "
            ...
            return HoloMap(self._obj[samples]).apply(
                apply_function, streams, link_inputs, link_dataset, dynamic, per_element, **kwargs
            )
            ...
            def apply_function(object, **kwargs):
                ...
                if method is None:
                    raise AttributeError(
                        f"Applied method {method_name} does not exist."
        ...
        kwargs = {k: v.param.value if isinstance(v, Widget) else v for k, v in kwargs.items()}
        ...
        params = {
            ...
            if isinstance(val, param.Parameter) and isinstance(val.owner, param.Parameterized)
        }

        dependent_kws = any(
            (isinstance(val, FunctionType) and hasattr(val, "_dinfo"))
            or util.is_param_method(val, has_deps=True)
        ...
        if dynamic is None:
            is_dynamic = (
                bool(streams)
                or isinstance(self._obj, DynamicMap)
                or util.is_param_method(apply_function, has_deps=True)
                or params
                or dependent_kws
            )
        else:
    ...
    def replace_dimensions(cls, dimensions, overrides):
        ...
        for d in dimensions:
            ...
            if override is None:
                ...
                raise ValueError(
                    "Dimension can only be overridden "
...
class Opts(metaclass=AccessorPipelineMeta):
    ...
    def __call__(self, *args, **kwargs):
        ...
        **kwargs : Keywords of options
        ...
        if self._mode is None:
            ...
            if apply_groups:
                msg = (
                    "Calling the .opts method with options broken down by options "
    ...
    def _dynamicmap_opts(self, *args, **kwargs):
        ...
        dmap = Dynamic(
            obj,
            operation=lambda obj, **dynkwargs: obj.opts(*args, **kwargs),
            streams=self._obj.streams,
            link_inputs=True,
        )
        if not clone:
    ...
    def __getitem__(self, item):
        ...
        if item in options:
            ...
            raise KeyError(f"{item!r} is not in opts. Valid items is {', '.join(options)}.")
