...
def test_box_select(serve_hv, BoundsTool, bound_slice, bound_attr):
    hv_scatter = hv.Scatter([1, 2, 3]).opts(tools=["box_select"], active_tools=["box_select"])
...
def test_lasso_select(serve_hv):
    hv_scatter = hv.Scatter([1, 2, 3]).opts(tools=["lasso_select"], active_tools=["lasso_select"])
...
def test_rangexy(serve_hv):
    ...
    wait_until(
        lambda: rangexy.x_range == expected_xrange and rangexy.y_range == expected_yrange, page
    )
...
def test_multi_axis_rangexy(serve_hv):
    ...
    wait_until(
        lambda: (
            np.testing.assert_almost_equal(s1.x_range, expected_xrange)
            and np.testing.assert_almost_equal(s1.y_range, expected_yrange1)
            and np.testing.assert_almost_equal(s2.y_range, expected_yrange2)
        ),
        page,
    )
...
def test_stream_subcoordinate_y_range(serve_hv, points):
    ...
    wait_until(
        lambda: stream.x_range == expected_xrange and stream.y_range == expected_yrange, page
    )
...
class TestPopup:
    ...
    def _get_popup_distances_relative_to_bbox(self, popup_box, plot_box):
        return {
            ...
            "right": abs(
                (popup_box["x"] + popup_box["width"]) - (plot_box["x"] + plot_box["width"])
            ),
            "top": abs(popup_box["y"] - plot_box["y"]),
            "bottom": abs(
                (popup_box["y"] + popup_box["height"]) - (plot_box["y"] + plot_box["height"])
            ),
        }
    ...
    @pytest.mark.parametrize(
        "popup_position",
        ["top_right", "top_left", "bottom_left", "bottom_right", "right", "left", "top", "bottom"],
    )
    ...
    def test_anchor_selection1d(self, serve_hv, points):
        ...
        hv.streams.Selection1D(
            source=points, popup=popup_form, popup_position="top", popup_anchor="top_right"
        )
    ...
    @pytest.mark.parametrize(
        ("tool", "tool_type"), [("box_select", BoundsXY), ("lasso_select", Lasso), ("tap", Tap)]
    )
    def test_anchor_tools(self, serve_hv, points, tool, tool_type):
        ...
        tool_type(
            source=points, popup=popup_form, popup_position="bottom", popup_anchor="bottom_right"
        )
        page, hv_plot = self._serve_plot(serve_hv, points)
    ...
    @pytest.mark.parametrize(
        ("tool", "tool_type"), [("box_select", BoundsXY), ("lasso_select", Lasso)]
    )
    @pytest.mark.parametrize(
        "popup_position",
        ["top_right", "top_left", "bottom_left", "bottom_right", "right", "left", "top", "bottom"],
...
def test_poly_point_visible(serve_panel, start_value):
    ...
    def points_layer(value):
        return hv.Points([(0, 0), (0.5, 0.5)]).opts(
            color="red", size=10, visible=value == "Points"
        )

    @pn.depends(select)
    def polygons_layer(value):
        return hv.Polygons([[(0.6, 0.1), (0.9, 0.1), (0.9, 0.9), (0.6, 0.9)]]).opts(
            visible=value == "Polygons"
        )
