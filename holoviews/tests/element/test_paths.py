...
class PathTests:
    def test_multi_path_list_constructor(self):
        ...
        assert_data_equal(path.dimension_values(0), np.array([0, 1, np.nan, 2, 3]))
        assert_data_equal(path.dimension_values(1), np.array([1, 2, np.nan, 3, 4]))

    def test_multi_path_cast_path(self):
        path = hv.Path([[(0, 1), (1, 2)], [(2, 3), (3, 4)]])
        path2 = hv.Path(path)
        assert path2.interface.multi
        assert_data_equal(path2.dimension_values(0), np.array([0, 1, np.nan, 2, 3]))
        assert_data_equal(path2.dimension_values(1), np.array([1, 2, np.nan, 3, 4]))

    def test_multi_path_tuple(self):
        path = hv.Path(([0, 1], [[1, 3], [2, 4]]))
        assert path.interface.multi
        assert_data_equal(path.dimension_values(0), np.array([0, 1, np.nan, 0, 1]))
        assert_data_equal(path.dimension_values(1), np.array([1, 2, np.nan, 3, 4]))
    ...
    def test_multi_path_unpack_multi_paths(self):
        path = hv.Path(
            [hv.Path([[(0, 1), (1, 2)]]), hv.Path([[(2, 3), (3, 4)], [(4, 5), (5, 6)]])]
        )
        assert path.interface.multi
        assert_data_equal(path.dimension_values(0), np.array([0, 1, np.nan, 2, 3, np.nan, 4, 5]))
        assert_data_equal(path.dimension_values(1), np.array([1, 2, np.nan, 3, 4, np.nan, 5, 6]))

    def test_single_path_list_constructor(self):
        path = hv.Path([(0, 1), (1, 2), (2, 3), (3, 4)])
        assert_data_equal(path.dimension_values(0), np.array([0, 1, 2, 3]))
        assert_data_equal(path.dimension_values(1), np.array([1, 2, 3, 4]))
