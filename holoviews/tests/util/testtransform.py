# -*- coding: utf-8 -*-
"""
Unit tests for dim transforms
"""
from __future__ import division

import numpy as np

from holoviews.core.data import Dataset
from holoviews.element.comparison import ComparisonTestCase
from holoviews.util.transform import dim


class TestDimTransforms(ComparisonTestCase):

    def setUp(self):
        self.linear_ints = np.arange(1, 11)
        self.linear_floats = np.arange(1, 11)/10.
        self.negative = -self.linear_floats
        self.repeating = ['A', 'B', 'C', 'A', 'B', 'C', 'A', 'B', 'C', 'A']
        self.dataset = Dataset(
            (self.linear_ints, self.linear_floats, self.negative, self.repeating),
            ['int', 'float', 'negative', 'categories']
        )

    # Unary operators

    def test_abs_transform(self):
        self.assertEqual(abs(dim('negative')).eval(self.dataset), self.linear_floats)

    def test_neg_transform(self):
        self.assertEqual(-dim('negative').eval(self.dataset), self.linear_floats)

    # Binary operators

    def test_add_transform(self):
        self.assertEqual((dim('float')+1).eval(self.dataset), self.linear_floats+1)

    def test_div_transform(self):
        self.assertEqual((dim('int')/10.).eval(self.dataset), self.linear_floats)

    def test_floor_div_transform(self):
        self.assertEqual((dim('int')//2).eval(self.dataset), self.linear_ints//2)

    def test_mod_transform(self):
        self.assertEqual((dim('int')%2).eval(self.dataset), self.linear_ints%2)

    def test_mul_transform(self):
        self.assertEqual((dim('float')*10.).eval(self.dataset), self.linear_ints)

    def test_pow_transform(self):
        self.assertEqual((dim('int')**2).eval(self.dataset), self.linear_ints**2)

    def test_sub_transform(self):
        self.assertEqual((dim('int')-10).eval(self.dataset), self.linear_ints-10)

    # Reverse binary operators

    def test_radd_transform(self):
        self.assertEqual((1+dim('float')).eval(self.dataset), 1+self.linear_floats)

    def test_rdiv_transform(self):
        self.assertEqual((10./dim('int')).eval(self.dataset), 10./self.linear_ints)

    def test_rfloor_div_transform(self):
        self.assertEqual((2//dim('int')).eval(self.dataset), 2//self.linear_ints)

    def test_rmod_transform(self):
        self.assertEqual((2%dim('int')).eval(self.dataset), 2%self.linear_ints)

    def test_rmul_transform(self):
        self.assertEqual((10.*dim('float')).eval(self.dataset), self.linear_ints)

    def test_rsub_transform(self):
        self.assertEqual((10-dim('int')).eval(self.dataset), 10-self.linear_ints)

    # NumPy operations

    def test_ufunc_transform(self):
        self.assertEqual(np.sin(dim('float')).eval(self.dataset), np.sin(self.linear_floats))

    def test_astype_transform(self):
        self.assertEqual(dim('int').astype(str).eval(self.dataset),
                         self.linear_ints.astype(str))

    def test_cumsum_transform(self):
        self.assertEqual(dim('float').cumsum().eval(self.dataset),
                         self.linear_floats.cumsum())

    def test_max_transform(self):
        self.assertEqual(dim('float').max().eval(self.dataset),
                         self.linear_floats.max())

    def test_min_transform(self):
        self.assertEqual(dim('float').min().eval(self.dataset),
                         self.linear_floats.min())
    
    def test_round_transform(self):
        self.assertEqual(dim('float').round().eval(self.dataset),
                         self.linear_floats.round())

    def test_sum_transform(self):
        self.assertEqual(dim('float').sum().eval(self.dataset),
                         self.linear_floats.sum())

    def test_std_transform(self):
        self.assertEqual(dim('float').std().eval(self.dataset),
                         self.linear_floats.std())
        
    def test_var_transform(self):
        self.assertEqual(dim('float').var().eval(self.dataset),
                         self.linear_floats.var())

    # Custom functions

    def test_norm_transform(self):
        self.assertEqual(dim('int').norm().eval(self.dataset),
                         (self.linear_ints-1)/9.)

    def test_bin_transform(self):
        self.assertEqual(dim('int').bin([0, 5, 10]).eval(self.dataset),
                         np.array([2.5, 2.5, 2.5, 2.5, 2.5, 7.5, 7.5, 7.5, 7.5, 7.5]))

    def test_bin_transform_with_labels(self):
        self.assertEqual(dim('int').bin([0, 5, 10], ['A', 'B']).eval(self.dataset),
                         np.array(['A', 'A', 'A', 'A', 'A', 'B', 'B', 'B', 'B', 'B']))

    def test_cat_transform_list(self):
        self.assertEqual(dim('categories').cat(['circle', 'square', 'triangle']).eval(self.dataset),
                         np.array((['circle', 'square', 'triangle']*3)+['circle']))

    def test_cat_transform_dict(self):
        self.assertEqual(dim('categories').cat({'A': 'circle', 'B': 'square', 'C': 'triangle'}).eval(self.dataset),
                         np.array((['circle', 'square', 'triangle']*3)+['circle']))

    def test_cat_transform_dict_with_empty(self):
        self.assertEqual(dim('categories').cat({'A': 'circle', 'B': 'square'}, empty='triangle').eval(self.dataset),
                         np.array((['circle', 'square', 'triangle']*3)+['circle']))

    # Complex expressions

    def test_multi_operator_expression(self):
        self.assertEqual((((dim('float')-2)*3)**2).eval(self.dataset),
                         ((self.linear_floats-2)*3)**2)

    def test_multi_dim_expression(self):
        self.assertEqual((dim('int')-dim('float')).eval(self.dataset),
                         self.linear_ints-self.linear_floats)

    # Repr method

    def test_dim_repr(self):
        self.assertEqual(repr(dim('float')), "'float'")

    def test_multi_operator_expression_repr(self):
        self.assertEqual(repr(((dim('float')-2)*3)**2),
                         "pow(mul(sub('float', 2), 3), 2)")

    # Applies method

    def test_multi_dim_expression_applies(self):
        self.assertEqual((dim('int')-dim('float')).applies(self.dataset),
                         True)

    def test_multi_dim_expression_not_applies(self):
        self.assertEqual((dim('foo')-dim('bar')).applies(self.dataset),
                         False)

    def test_multi_dim_expression_partial_applies(self):
        self.assertEqual((dim('int')-dim('bar')).applies(self.dataset),
                         False)
