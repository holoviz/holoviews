...
class RoundTripTests:
    ...
    def test_polygon_roundtrip(self):
        ...
        expected = hv.Polygons(
            [{"x": [*xs, 1], "y": [*ys, 2], "z": 0}, {"x": [3, *xs], "y": [7, *ys], "z": 1}],
            ["x", "y"],
            "z",
            datatype=["multitabular"],
        )
...
class SpatialPandasTest(GeomTests, RoundTripTests):
    ...
    def test_point_constructor(self):
        points = hv.Points(
            [{"x": 0, "y": 1}, {"x": 1, "y": 0}], ["x", "y"], datatype=[self.datatype]
        )
        assert isinstance(points.data.geometry.dtype, sgeom.PointDtype)
    ...
    def test_multi_point_constructor(self):
        ...
        ys = [2, 0, 7, 4]
        points = hv.Points(
            [{"x": xs, "y": ys}, {"x": xs[::-1], "y": ys[::-1]}],
            ["x", "y"],
            datatype=[self.datatype],
        )
        assert isinstance(points.data.geometry.dtype, sgeom.MultiPointDtype)
        assert_data_equal(points.data.iloc[0, 0].buffer_values, np.array([1, 2, 2, 0, 3, 7, 2, 4]))
        assert_data_equal(points.data.iloc[1, 0].buffer_values, np.array([2, 4, 3, 7, 2, 0, 1, 2]))
    ...
    def test_line_constructor(self):
        ...
        assert isinstance(path.data.geometry.dtype, sgeom.LineDtype)
        assert_data_equal(path.data.iloc[0, 0].buffer_values, np.array([1, 2, 2, 0, 3, 7]))
        assert_data_equal(path.data.iloc[1, 0].buffer_values, np.array([3, 7, 2, 0, 1, 2]))
    ...
    def test_multi_line_constructor(self):
        ...
        assert isinstance(path.data.geometry.dtype, sgeom.MultiLineDtype)
        assert_data_equal(
            path.data.iloc[0, 0].buffer_values, np.array([1, 2, 2, 0, 3, 7, 6, 7, 7, 5, 3, 2])
        )
        assert_data_equal(
    ...
    def test_polygon_constructor(self):
        ...
        path = hv.Polygons(
            [{"x": xs, "y": ys, "holes": holes}, {"x": xs[::-1], "y": ys[::-1]}],
            ["x", "y"],
            datatype=[self.datatype],
        )
        ...
        assert_data_equal(path.data.iloc[1, 0].buffer_values, np.array([3, 7, 1, 2, 2, 0, 3, 7]))
