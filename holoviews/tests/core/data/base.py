...
class HomogeneousColumnTests:
    ...
    def init_column_data(self):
        ...
        self.dataset_hm = hv.Dataset(
            self.frame({"x": self.xs, "y": self.y_ints}), kdims=["x"], vdims=["y"]
        )
        self.dataset_hm_alias = hv.Dataset(
            self.frame({"x": self.xs, "y": self.y_ints}), kdims=[("x", "X")], vdims=[("y", "Y")]
        )
    ...
    def test_dataset_array_init_hm(self):
        dataset = hv.Dataset(np.column_stack([self.xs, self.xs_2]), kdims=["x"], vdims=["x2"])
        assert isinstance(dataset.data, self.data_type)

    def test_dataset_dtypes(self):
        assert self.dataset_hm.interface.dtype(self.dataset_hm, "x") == np.dtype(int)
        assert self.dataset_hm.interface.dtype(self.dataset_hm, "y") == np.dtype(int)

    def test_dataset_array_init_hm_tuple_dims(self):
        dataset = hv.Dataset(
            np.column_stack([self.xs, self.xs_2]), kdims=[("x", "X")], vdims=[("x2", "X2")]
        )
        assert isinstance(dataset.data, self.data_type)
    ...
    def test_dataset_dataframe_init_hm(self):
        ...
        dataset = hv.Dataset(
            self.frame({"x": self.xs, "x2": self.xs_2}), kdims=["x"], vdims=["x2"]
        )
        assert isinstance(dataset.data, self.data_type)
    ...
    def test_dataset_dataframe_init_hm_alias(self):
        ...
        if self.frame is dict:
            pytest.skip("Only valid for non-dict frame")
        dataset = hv.Dataset(
            self.frame({"x": self.xs, "x2": self.xs_2}),
            kdims=[("x", "X-label")],
            vdims=[("x2", "X2-label")],
        )
        assert isinstance(dataset.data, self.data_type)
    ...
    def test_dataset_shape(self):
        assert self.dataset_hm.shape == (11, 2)

    def test_dataset_range(self):
        assert self.dataset_hm.range("y") == (0, 20)

    def test_dataset_closest(self):
        closest = self.dataset_hm.closest([0.51, 1, 9.9])
        assert closest == [1.0, 1.0, 10.0]

    # Operations

    def test_dataset_sort_hm(self):
        ds = hv.Dataset(
            ([2, 2, 1], [2, 1, 2], [0.1, 0.2, 0.3]), kdims=["x", "y"], vdims=["z"]
        ).sort()
        ds_sorted = hv.Dataset(
            ([1, 2, 2], [2, 1, 2], [0.3, 0.2, 0.1]), kdims=["x", "y"], vdims=["z"]
        )
        assert_element_equal(ds.sort(), ds_sorted)

    def test_dataset_sort_reverse_hm(self):
        ds = hv.Dataset(
            ([2, 1, 2, 1], [2, 2, 1, 1], [0.1, 0.2, 0.3, 0.4]), kdims=["x", "y"], vdims=["z"]
        )
        ds_sorted = hv.Dataset(
            ([2, 2, 1, 1], [2, 1, 2, 1], [0.1, 0.3, 0.2, 0.4]), kdims=["x", "y"], vdims=["z"]
        )
        assert_element_equal(ds.sort(reverse=True), ds_sorted)

    def test_dataset_sort_vdim_hm(self):
        xs_2 = np.array(self.xs_2)
        dataset = hv.Dataset(np.column_stack([self.xs, -xs_2]), kdims=["x"], vdims=["y"])
        dataset_sorted = hv.Dataset(
            np.column_stack([self.xs[::-1], -xs_2[::-1]]), kdims=["x"], vdims=["y"]
        )
    ...
    def test_dataset_sort_reverse_vdim_hm(self):
        xs_2 = np.array(self.xs_2)
        dataset = hv.Dataset(np.column_stack([self.xs, -xs_2]), kdims=["x"], vdims=["y"])
        dataset_sorted = hv.Dataset(np.column_stack([self.xs, -xs_2]), kdims=["x"], vdims=["y"])
        assert_element_equal(dataset.sort("y", reverse=True), dataset_sorted)

    def test_dataset_sort_vdim_hm_alias(self):
        xs_2 = np.array(self.xs_2)
        dataset = hv.Dataset(
            np.column_stack([self.xs, -xs_2]), kdims=[("x", "X-label")], vdims=[("y", "Y-label")]
        )
        dataset_sorted = hv.Dataset(
            np.column_stack([self.xs[::-1], -xs_2[::-1]]),
            kdims=[("x", "X-label")],
            vdims=[("y", "Y-label")],
        )
        assert_element_equal(dataset.sort("y"), dataset_sorted)
    ...
    def test_dataset_array_hm(self):
        assert_data_equal(self.dataset_hm.array(), np.column_stack([self.xs, self.y_ints]))

    def test_dataset_array_hm_alias(self):
        assert_data_equal(self.dataset_hm_alias.array(), np.column_stack([self.xs, self.y_ints]))

    def test_dataset_add_dimensions_value_hm(self):
        table = self.dataset_hm.add_dimension("z", 1, 0)
        assert table.kdims[1] == "z"
        assert_data_equal(np.zeros(table.shape[0]), table.dimension_values("z"))
    ...
    def test_dataset_slice_hm(self):
        dataset_slice = hv.Dataset(
            self.frame({"x": range(5, 9), "y": [2 * i for i in range(5, 9)]}),
            kdims=["x"],
            vdims=["y"],
        )
    ...
    def test_dataset_2D_reduce_hm(self):
        dataset = hv.Dataset(
            self.frame({"x": self.xs, "y": self.y_ints, "z": [el**2 for el in self.y_ints]}),
            kdims=["x", "y"],
            vdims=["z"],
        )
    ...
    def test_dataset_2D_aggregate_partial_hm(self):
        ...
        dataset = hv.Dataset(
            self.frame({"x": self.xs, "y": self.y_ints, "z": z_ints}),
            kdims=["x", "y"],
            vdims=["z"],
        )
    ...
    def test_dataset_iloc_slice_rows(self):
        ...
        table = hv.Dataset(
            {"x": self.xs[1:4], "y": self.y_ints[1:4]},
            kdims=["x"],
            vdims=["y"],
            datatype=["dictionary"],
        )
    ...
    def test_dataset_iloc_slice_rows_slice_cols(self):
        ...
        table = hv.Dataset({"y": self.y_ints[1:4]}, kdims=[], vdims=["y"], datatype=["dictionary"])
        assert_element_equal(sliced, table)
    ...
    def test_dataset_iloc_list_rows(self):
        ...
        table = hv.Dataset(
            {"x": self.xs[[0, 2]], "y": self.y_ints[[0, 2]]},
            kdims=["x"],
            vdims=["y"],
            datatype=["dictionary"],
        )
    ...
    def test_dataset_iloc_list_cols(self):
        ...
        table = hv.Dataset(
            {"x": self.xs, "y": self.y_ints}, kdims=["x"], vdims=["y"], datatype=["dictionary"]
        )
        assert_element_equal(sliced, table)
...
class HeterogeneousColumnTests(HomogeneousColumnTests):
    ...
    def init_column_data(self):
        ...
        self.dataset_ht = hv.Dataset(
            self.frame({"x": self.xs, "y": self.ys}), kdims=["x"], vdims=["y"]
        )
    ...
    def test_dataset_dataframe_init_ht_alias(self):
        ...
        dataset = hv.Dataset(
            self.frame({"x": self.xs, "y": self.ys}), kdims=[("x", "X")], vdims=[("y", "Y")]
        )
        assert isinstance(dataset.data, self.data_type)
    ...
    def test_dataset_simple_zip_init_alias(self):
        dataset = hv.Dataset(
            zip(self.xs, self.ys, strict=None), kdims=[("x", "X")], vdims=[("y", "Y")]
        )
        assert isinstance(dataset.data, self.data_type)

    def test_dataset_zip_init(self):
        dataset = hv.Dataset(
            zip(self.gender, self.age, self.weight, self.height, strict=None),
            kdims=self.kdims,
            vdims=self.vdims,
        )
        assert isinstance(dataset.data, self.data_type)
    ...
    def test_dataset_zip_init_alias(self):
        dataset = self.alias_table.clone(
            zip(self.gender, self.age, self.weight, self.height, strict=None)
        )
        assert isinstance(dataset.data, self.data_type)
    ...
    def test_dataset_dict_init_alias(self):
        dataset = hv.Dataset(
            dict(zip(self.xs, self.ys, strict=None)), kdims=[("a", "A")], vdims=[("b", "B")]
        )
        assert isinstance(dataset.data, self.data_type)
    ...
    def test_dataset_sort_vdim_ht(self):
        dataset = hv.Dataset({"x": self.xs, "y": -self.ys}, kdims=["x"], vdims=["y"])
        dataset_sorted = hv.Dataset(
            {"x": self.xs[::-1], "y": -self.ys[::-1]}, kdims=["x"], vdims=["y"]
        )
        assert_element_equal(dataset.sort("y"), dataset_sorted)
    ...
    def test_dataset_2D_reduce_ht(self):
        reduced = hv.Dataset(
            self.frame({"Weight": [14.333333333333334], "Height": [0.73333333333333339]}),
            kdims=[],
            vdims=self.vdims,
        )
    ...
    def test_dataset_2D_partial_reduce_ht(self):
        dataset = hv.Dataset(
            self.frame({"x": self.xs, "y": self.ys, "z": self.zs}), kdims=["x", "y"], vdims=["z"]
        )
        expected = hv.Dataset({"x": self.xs, "z": self.zs}, kdims=["x"], vdims=["z"])
    ...
    def test_dataset_aggregate_string_types(self):
        ds = hv.Dataset(
            self.frame({"Gender": ["M", "M"], "Weight": [20, 10], "Name": ["Peter", "Matt"]}),
            kdims="Gender",
            vdims=["Weight", "Name"],
        )
        aggregated = hv.Dataset(
            {"Gender": ["M"], "Weight": [15]}, kdims="Gender", vdims=["Weight"]
        )
    ...
    def test_dataset_aggregate_string_types_size(self):
        ...
        aggregated = hv.Dataset(
            {"Gender": ["M"], "Weight": [2], "Name": [2]}, kdims="Gender", vdims=["Weight", "Name"]
        )
    ...
    def test_dataset_empty_aggregate_with_spreadfn(self):
        dataset = hv.Dataset([], kdims=self.kdims, vdims=self.vdims)
        aggregated = hv.Dataset(
            [], kdims=self.kdims[:1], vdims=[d for vd in self.vdims for d in [vd, vd + "_std"]]
        )
    ...
    def test_dataset_groupby_dynamic(self):
        ...
        assert_element_equal(grouped_dataset["F"], self.table.select(Gender="F").reindex(["Age"]))
    ...
    def test_dataset_groupby_dynamic_alias(self):
        ...
        assert_element_equal(
            grouped_dataset["M"], self.alias_table.select(gender="M").reindex(["Age"])
        )
        assert_element_equal(
            grouped_dataset["F"], self.alias_table.select(gender="F").reindex(["Age"])
        )
    ...
    def test_redim_with_extra_dimension(self):
        ...
        assert_data_equal(redimmed.dimension_values("Time"), self.dataset_ht.dimension_values("x"))

    # Indexing

    def test_dataset_index_row_gender_female(self):
        indexed = hv.Dataset(
            {"Gender": ["F"], "Age": [12], "Weight": [10], "Height": [0.8]},
            kdims=self.kdims,
            vdims=self.vdims,
        )
        row = self.table["F", :]
    ...
    def test_dataset_index_rows_gender_male(self):
        row = self.table["M", :]
        indexed = hv.Dataset(
            {"Gender": ["M", "M"], "Age": [10, 16], "Weight": [15, 18], "Height": [0.8, 0.6]},
            kdims=self.kdims,
            vdims=self.vdims,
        )
        assert_element_equal(row, indexed)
    ...
    def test_dataset_index_value1(self):
        assert self.table["F", 12, "Weight"] == 10
    ...
    def test_dataset_value_dim_index(self):
        ...
        indexed = hv.Dataset(
            {"Gender": ["M", "M", "F"], "Age": [10, 16, 12], "Weight": [15, 18, 10]},
            kdims=self.kdims,
            vdims=self.vdims[:1],
        )
        assert_element_equal(row, indexed)
    ...
    def test_dataset_iloc_slice_rows(self):
        ...
        table = hv.Dataset(
            {
                "Gender": self.gender[1:2],
                "Age": self.age[1:2],
                "Weight": self.weight[1:2],
    ...
    def test_dataset_iloc_slice_rows_slice_cols(self):
        ...
        table = hv.Dataset(
            {"Age": self.age[1:2], "Weight": self.weight[1:2]},
            kdims=self.kdims[1:],
            vdims=self.vdims[:1],
        )
        assert_element_equal(sliced, table)
    ...
    def test_dataset_iloc_slice_rows_list_cols(self):
        sliced = self.table.iloc[1:2, [1, 3]]
        table = hv.Dataset(
            {"Age": self.age[1:2], "Height": self.height[1:2]},
            kdims=self.kdims[1:],
            vdims=self.vdims[1:],
        )
        assert_element_equal(sliced, table)
    ...
    def test_dataset_iloc_list_rows_list_cols(self):
        ...
        table = hv.Dataset(
            {"Gender": self.gender[[0, 2]], "Weight": self.weight[[0, 2]]},
            kdims=self.kdims[:1],
            vdims=self.vdims[:1],
        )
        assert_element_equal(sliced, table)
    ...
    def test_dataset_iloc_list_rows_slice_cols(self):
        ...
        table = hv.Dataset(
            {"Age": self.age[[0, 2]], "Weight": self.weight[[0, 2]]},
            kdims=self.kdims[1:],
            vdims=self.vdims[:1],
        )
        assert_element_equal(sliced, table)
    ...
    def test_dataset_iloc_index_rows_index_cols(self):
        ...
        assert indexed == self.age[1]

    def test_dataset_iloc_index_rows_slice_cols(self):
        indexed = self.table.iloc[1, 1:3]
        table = hv.Dataset(
            {"Age": self.age[[1]], "Weight": self.weight[[1]]},
            kdims=self.kdims[1:],
            vdims=self.vdims[:1],
        )
        assert_element_equal(indexed, table)
    ...
    def test_dataset_iloc_list_cols(self):
        sliced = self.table.iloc[:, [0, 2]]
        table = hv.Dataset(
            {"Gender": self.gender, "Weight": self.weight},
            kdims=self.kdims[:1],
            vdims=self.vdims[:1],
        )
        assert_element_equal(sliced, table)
    ...
    def test_dataset_array_ht(self):
        assert_data_equal(self.dataset_ht.array(), np.column_stack([self.xs, self.ys]))

    # Transforms

    def test_dataset_transform_replace_ht(self):
        transformed = self.table.transform(
            Age=hv.dim("Age") ** 2, Weight=hv.dim("Weight") * 2, Height=hv.dim("Height") / 2.0
        )
        expected = hv.Dataset(
            {
                "Gender": self.gender,
    ...
    def test_select_with_neighbor(self):
        ...
        select_neighbor = self.table.interface._select_mask_neighbor(
            self.table.dataset, dict(Weight=18)
        )
...
class ScalarColumnTests:
    ...
    def test_dataset_scalar_select_expr(self):
        ds = hv.Dataset({"A": 1, "B": np.arange(10)}, kdims=["A", "B"])
        assert_data_equal(
            ds.select(selection_expr=hv.dim("A") == 1).dimension_values("B"), np.arange(10)
        )
    ...
    def test_dataset_scalar_empty_select_expr(self):
        ds = hv.Dataset({"A": 1, "B": np.arange(10)}, kdims=["A", "B"])
        assert_data_equal(
            ds.select(selection_expr=hv.dim("A") == 0).dimension_values("B"), np.array([])
        )
    ...
    def test_dataset_scalar_iloc(self):
        ds = hv.Dataset({"A": 1, "B": np.arange(10)}, kdims=["A", "B"])
        assert_element_equal(
            ds.iloc[:5], hv.Dataset({"A": 1, "B": np.arange(5)}, kdims=["A", "B"])
        )
...
class GriddedInterfaceTests:
    ...
    def test_canonical_vdim(self):
        ...
        assert_data_equal(dataset.dimension_values("z", flat=False), canonical)

    def test_gridded_dtypes(self):
        ds = self.dataset_grid
        assert ds.interface.dtype(ds, "x") == np.dtype(int)
        assert ds.interface.dtype(ds, "y") == np.float64
    ...
    def test_select_slice(self):
        ...
        assert_element_equal(self.dataset_grid.select(y=slice(0, 0.25)), ds)
    ...
    def test_select_slice_as_dimension_dict(self):
        ds = self.element((self.grid_xs, self.grid_ys[:2], self.grid_zs[:2]), ["x", "y"], ["z"])
        assert_element_equal(
            self.dataset_grid.select({self.dataset_grid.kdims[1]: slice(0, 0.25)}), ds
        )

    def test_select_tuple(self):
        ds = self.element((self.grid_xs, self.grid_ys[:2], self.grid_zs[:2]), ["x", "y"], ["z"])
        assert_element_equal(self.dataset_grid.select(y=(0, 0.25)), ds)
    ...
    def test_dataset_ndloc_index(self):
        ...
        assert ds.ndloc[0, 0] == arr[0, 0]

    def test_dataset_ndloc_index_inverted_x(self):
        xs, ys = np.linspace(0.12, 0.81, 10), np.linspace(0.12, 0.391, 5)
        arr = np.arange(10) * np.arange(5)[np.newaxis].T
        ds = self.element(
            (xs[::-1], ys, arr), kdims=["x", "y"], vdims=["z"], datatype=[self.datatype]
        )
        assert ds.ndloc[0, 0] == arr[0, 9]

    def test_dataset_ndloc_index_inverted_y(self):
        xs, ys = np.linspace(0.12, 0.81, 10), np.linspace(0.12, 0.391, 5)
        arr = np.arange(10) * np.arange(5)[np.newaxis].T
        ds = self.element(
            (xs, ys[::-1], arr), kdims=["x", "y"], vdims=["z"], datatype=[self.datatype]
        )
        assert ds.ndloc[0, 0] == arr[4, 0]

    def test_dataset_ndloc_index_inverted_xy(self):
        xs, ys = np.linspace(0.12, 0.81, 10), np.linspace(0.12, 0.391, 5)
        arr = np.arange(10) * np.arange(5)[np.newaxis].T
        ds = self.element(
            (xs[::-1], ys[::-1], arr), kdims=["x", "y"], vdims=["z"], datatype=[self.datatype]
        )
        assert ds.ndloc[0, 0] == arr[4, 9]

    def test_dataset_ndloc_index2(self):
        xs, ys = np.linspace(0.12, 0.81, 10), np.linspace(0.12, 0.391, 5)
        arr = np.arange(10) * np.arange(5)[np.newaxis].T
        ds = self.element((xs, ys, arr), kdims=["x", "y"], vdims=["z"], datatype=[self.datatype])
        assert ds.ndloc[4, 9] == arr[4, 9]
    ...
    def test_dataset_dim_vals_grid_kdims_xs(self):
        assert_data_equal(self.dataset_grid.dimension_values(0, expanded=False), np.array([0, 1]))

    def test_dataset_dim_vals_grid_kdims_xs_alias(self):
        assert_data_equal(
            self.dataset_grid_alias.dimension_values("x", expanded=False), np.array([0, 1])
        )
        assert_data_equal(
            self.dataset_grid_alias.dimension_values("X", expanded=False), np.array([0, 1])
        )
    ...
    def test_dataset_dim_vals_grid_kdims_xs_inv(self):
        assert_data_equal(
            self.dataset_grid_inv.dimension_values(0, expanded=False), np.array([0, 1])
        )

    def test_dataset_dim_vals_grid_kdims_expanded_xs_flat(self):
        expanded_xs = np.array([0, 0, 0, 1, 1, 1])
        assert_data_equal(self.dataset_grid.dimension_values(0), expanded_xs)

    def test_dataset_dim_vals_grid_kdims_expanded_xs_flat_inv(self):
        expanded_xs = np.array([0, 0, 0, 1, 1, 1])
        assert_data_equal(self.dataset_grid_inv.dimension_values(0), expanded_xs)

    def test_dataset_dim_vals_grid_kdims_expanded_xs(self):
        expanded_xs = np.array([[0, 1], [0, 1], [0, 1]])
        assert_data_equal(self.dataset_grid.dimension_values(0, flat=False), expanded_xs)

    def test_dataset_dim_vals_grid_kdims_expanded_xs_inv(self):
        expanded_xs = np.array([[0, 1], [0, 1], [0, 1]])
        assert_data_equal(self.dataset_grid_inv.dimension_values(0, flat=False), expanded_xs)

    def test_dataset_dim_vals_grid_kdims_ys(self):
        assert_data_equal(
            self.dataset_grid.dimension_values(1, expanded=False), np.array([0.1, 0.2, 0.3])
        )

    def test_dataset_dim_vals_grid_kdims_ys_inv(self):
        assert_data_equal(
            self.dataset_grid_inv.dimension_values(1, expanded=False), np.array([0.1, 0.2, 0.3])
        )

    def test_dataset_dim_vals_grid_kdims_expanded_ys_flat(self):
        expanded_ys = np.array([0.1, 0.2, 0.3, 0.1, 0.2, 0.3])
        assert_data_equal(self.dataset_grid.dimension_values(1), expanded_ys)

    def test_dataset_dim_vals_grid_kdims_expanded_ys_flat_inv(self):
        expanded_ys = np.array([0.1, 0.2, 0.3, 0.1, 0.2, 0.3])
        assert_data_equal(self.dataset_grid_inv.dimension_values(1), expanded_ys)

    def test_dataset_dim_vals_grid_kdims_expanded_ys(self):
        expanded_ys = np.array([[0.1, 0.1], [0.2, 0.2], [0.3, 0.3]])
        assert_data_equal(self.dataset_grid.dimension_values(1, flat=False), expanded_ys)

    def test_dataset_dim_vals_grid_kdims_expanded_ys_inv(self):
        expanded_ys = np.array([[0.1, 0.1], [0.2, 0.2], [0.3, 0.3]])
        assert_data_equal(self.dataset_grid_inv.dimension_values(1, flat=False), expanded_ys)

    def test_dataset_dim_vals_dimensions_match_shape(self):
        output = {self.dataset_grid.dimension_values(i, flat=False).shape for i in range(3)}
        assert len(output) == 1

    def test_dataset_dim_vals_dimensions_match_shape_inv(self):
        output = {self.dataset_grid_inv.dimension_values(i, flat=False).shape for i in range(3)}
        assert len(output) == 1

    def test_dataset_dim_vals_grid_vdims_zs_flat(self):
        expanded_zs = np.array([0, 2, 4, 1, 3, 5])
        assert_data_equal(self.dataset_grid.dimension_values(2), expanded_zs)

    def test_dataset_dim_vals_grid_vdims_zs_flat_inv(self):
        expanded_zs = np.array([5, 3, 1, 4, 2, 0])
        assert_data_equal(self.dataset_grid_inv.dimension_values(2), expanded_zs)

    def test_dataset_dim_vals_grid_vdims_zs(self):
        expanded_zs = np.array([[0, 1], [2, 3], [4, 5]])
        assert_data_equal(self.dataset_grid.dimension_values(2, flat=False), expanded_zs)

    def test_dataset_dim_vals_grid_vdims_zs_inv(self):
        expanded_zs = np.array([[5, 4], [3, 2], [1, 0]])
        assert_data_equal(self.dataset_grid_inv.dimension_values(2, flat=False), expanded_zs)
    ...
    def test_aggregate_2d_with_spreadfn(self):
        ...
        example = hv.Dataset(
            (range(5), array.mean(axis=0), array.std(axis=0)), "x", ["z", "z_std"]
        )
    ...
    def test_concat_grid_3d(self):
        ...
        hmap = hv.HoloMap(
            {
                (i, j): self.element((range(5), range(4), array[:, :, j, i]), ["x", "y"], "z")
                for i in range(2)
                for j in range(3)
            },
            ["A", "B"],
