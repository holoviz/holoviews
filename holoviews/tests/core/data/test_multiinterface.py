...
class GeomTests:
    ...
    def test_array_dataset_add_dimension_scalar(self):
        arrays = [np.column_stack([np.arange(i, i + 2), np.arange(i, i + 2)]) for i in range(2)]
        mds = hv.Path(arrays, kdims=["x", "y"], datatype=[self.datatype]).add_dimension(
            "A", 0, "Scalar", True
        )
        assert mds.interface is self.interface
        assert_element_equal(
            mds,
            hv.Path([{("x", "y"): arrays[i], "A": "Scalar"} for i in range(2)], ["x", "y"], "A"),
        )
    ...
    def test_dict_dataset_add_dimension_scalar(self):
        ...
        assert_element_equal(
            mds,
            hv.Path(
                [dict(arrays[i], A="Scalar") for i in range(2)],
                ["x", "y"],
                "A",
    ...
    def test_dict_dataset_add_dimension_values(self):
        ...
        assert_element_equal(
            mds,
            hv.Path(
                [dict(arrays[i], A=i) for i in range(2)],
                ["x", "y"],
                "A",
                datatype=["multitabular"],
            ),
    ...
    def test_array_values(self):
        ...
        mds = hv.Path(arrays, kdims=["x", "y"], datatype=[self.datatype])
        assert mds.interface is self.interface
        assert_data_equal(mds.dimension_values(0), np.array([0.0, 1, np.nan, 1, 2]))
    ...
    def test_scalar_value_isscalar_per_geom(self):
        path = hv.Path(
            [
                {"x": [1, 2, 3, 4, 5], "y": [0, 0, 1, 1, 2], "value": 0},
    ...
    def test_unique_values_isscalar_per_geom(self):
        path = hv.Path(
            [
                {"x": [1, 2, 3, 4, 5], "y": [0, 0, 1, 1, 2], "value": np.full(5, 0)},
    ...
    def test_varying_values_not_isscalar_per_geom(self):
        path = hv.Path(
            [
                {"x": [1, 2, 3, 4, 5], "y": [0, 0, 1, 1, 2], "value": np.arange(5)},
                {"x": [5, 4, 3, 2, 1], "y": [2, 2, 1, 1, 0], "value": np.full(5, 1)},
            ],
            vdims="value",
            datatype=[self.datatype],
    ...
    def test_scalar_value_dimension_values_expanded(self):
        ...
        assert path.interface is self.interface
        assert_data_equal(
            path.dimension_values("value"), np.array([0, 0, 0, 0, 0, np.nan, 1, 1, 1, 1, 1])
        )
    ...
    def test_scalar_value_dimension_values_not_expanded(self):
        ...
        assert path.interface is self.interface
        assert_data_equal(path.dimension_values("value", expanded=False), np.array([0, 1]))
    ...
    def test_varying_value_dimension_values_expanded(self):
        ...
        assert path.interface is self.interface
        assert_data_equal(
            path.dimension_values("value"), np.array([0, 1, 2, 3, 4, np.nan, 1, 1, 1, 1, 1])
        )
    ...
    def test_mixed_dims_raises(self):
        arrays = [
            {"x": range(10), "y" if j else "z": range(10)} for i in range(2) for j in range(2)
        ]
    ...
    def test_multi_polygon_iloc_index_row(self):
        ...
        expected = hv.Polygons(
            [{"x": xs, "y": ys, "holes": holes, "z": 1}], ["x", "y"], "z", datatype=[self.datatype]
        )
    ...
    def test_multi_polygon_iloc_slice_rows(self):
        ...
        expected = hv.Polygons(
            [{"x": xs[::-1], "y": ys[::-1], "z": 2}, {"x": xs, "y": ys, "holes": holes, "z": 3}],
            ["x", "y"],
            "z",
            datatype=[self.datatype],
        )
    ...
    def test_polygon_expanded_values(self):
        xs = [1, 2, 3]
        ys = [2, 0, 7]
        poly = hv.Polygons([{"x": xs, "y": ys, "z": 1}], ["x", "y"], "z", datatype=[self.datatype])
    ...
    def test_select_from_multi_polygons_with_scalar(self):
        ...
        expected = hv.Polygons(
            [{"x": xs[::-1], "y": ys[::-1], "z": 2}], ["x", "y"], "z", datatype=[self.datatype]
        )
        assert poly.interface is self.interface
    ...
    def test_select_from_multi_polygons_with_slice(self):
        ...
        expected = hv.Polygons(
            [{"x": xs[::-1], "y": ys[::-1], "z": 2}, {"x": xs[:3], "y": ys[:3], "z": 3}],
            ["x", "y"],
            "z",
            datatype=[self.datatype],
        )
    ...
    def test_select_from_multi_polygons_with_list(self):
        ...
        expected = hv.Polygons(
            [{"x": xs, "y": ys, "holes": holes, "z": 1}, {"x": xs[:3], "y": ys[:3], "z": 3}],
            ["x", "y"],
            "z",
            datatype=[self.datatype],
        )
    ...
    def test_sort_by_value(self):
        path = hv.Path(
            [
                {"x": [1, 2, 3, 4, 5], "y": [0, 0, 1, 1, 2], "value": 1},
