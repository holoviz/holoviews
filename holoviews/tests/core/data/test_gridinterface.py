...
class BaseGridInterfaceTests(GriddedInterfaceTests, HomogeneousColumnTests, InterfaceTests):
    ...
    def test_dataset_dataframe_init_hm(self):
        with pytest.raises(DataError):
            hv.Dataset(pd.DataFrame({"x": self.xs, "x2": self.xs_2}), kdims=["x"], vdims=["x2"])
    ...
    def test_dataset_2D_columnar_shape(self, rng):
        array = rng.random((11, 11))
        dataset = hv.Dataset(
            {"x": self.xs, "y": self.y_ints, "z": array}, kdims=["x", "y"], vdims=["z"]
        )
        assert dataset.shape == (11 * 11, 3)

    def test_dataset_2D_gridded_shape(self, rng):
        array = rng.random((12, 11))
        dataset = hv.Dataset(
            {"x": self.xs, "y": range(12), "z": array}, kdims=["x", "y"], vdims=["z"]
        )
        assert dataset.interface.shape(dataset, gridded=True) == (12, 11)

    def test_dataset_2D_aggregate_partial_hm(self, rng):
        array = rng.random((11, 11))
        dataset = hv.Dataset(
            {"x": self.xs, "y": self.y_ints, "z": array}, kdims=["x", "y"], vdims=["z"]
        )
        assert_element_equal(
            ...
            hv.Dataset({"x": self.xs, "z": np.mean(array, axis=0)}, kdims=["x"], vdims=["z"]),
        )

    def test_dataset_2D_aggregate_partial_hm_alias(self, rng):
        array = rng.random((11, 11))
        dataset = hv.Dataset(
            {"x": self.xs, "y": self.y_ints, "z": array},
            kdims=[("x", "X"), ("y", "Y")],
            vdims=[("z", "Z")],
        )
        assert_element_equal(
            dataset.aggregate(["X"], np.mean),
            hv.Dataset(
                {"x": self.xs, "z": np.mean(array, axis=0)}, kdims=[("x", "X")], vdims=[("z", "Z")]
            ),
        )

    def test_dataset_2D_reduce_hm(self, rng):
        array = rng.random((11, 11))
        dataset = hv.Dataset(
            {"x": self.xs, "y": self.y_ints, "z": array}, kdims=["x", "y"], vdims=["z"]
        )
    ...
    def test_dataset_2D_reduce_hm_alias(self, rng):
        array = rng.random((11, 11))
        dataset = hv.Dataset(
            {"x": self.xs, "y": self.y_ints, "z": array},
            kdims=[("x", "X"), ("y", "Y")],
    ...
    def test_dataset_groupby_dynamic(self, rng):
        array = rng.random((11, 11))
        dataset = hv.Dataset(
            {"x": self.xs, "y": self.y_ints, "z": array}, kdims=["x", "y"], vdims=["z"]
        )
        ...
        first = hv.Dataset({"y": self.y_ints, "z": array[:, 0]}, kdims=["y"], vdims=["z"])
        assert_element_equal(grouped[0], first)

    def test_dataset_groupby_dynamic_alias(self, rng):
        array = rng.random((11, 11))
        dataset = hv.Dataset(
            {"x": self.xs, "y": self.y_ints, "z": array},
            kdims=[("x", "X"), ("y", "Y")],
        ...
        with DatatypeContext([self.datatype, "dictionary", "dataframe"], dataset):
            grouped = dataset.groupby("X", dynamic=True)
        first = hv.Dataset(
            {"y": self.y_ints, "z": array[:, 0]}, kdims=[("y", "Y")], vdims=[("z", "Z")]
        )
        assert_element_equal(grouped[0], first)

    def test_dataset_groupby_multiple_dims(self, rng):
        dataset = hv.Dataset(
            (range(8), range(8), range(8), range(8), rng.random((8, 8, 8, 8))),
            kdims=["a", "b", "c", "d"],
            vdims=["Value"],
        )
        grouped = dataset.groupby(["c", "d"])
        keys = list(product(range(8), range(8)))
        assert list(grouped.keys()) == keys
        for c, d in keys:
            assert_element_equal(grouped[c, d], dataset.select(c=c, d=d).reindex(["a", "b"]))

    def test_dataset_groupby_drop_dims(self, rng):
        array = rng.random((3, 20, 10))
        ds = hv.Dataset(
            {"x": range(10), "y": range(20), "z": range(3), "Val": array},
            kdims=["x", "y", "z"],
            vdims=["Val"],
        )
    ...
    def test_dataset_groupby_drop_dims_dynamic(self, rng):
        array = rng.random((3, 20, 10))
        ds = hv.Dataset(
            {"x": range(10), "y": range(20), "z": range(3), "Val": array},
            kdims=["x", "y", "z"],
    ...
    def test_dataset_groupby_drop_dims_with_vdim(self, rng):
        array = rng.random((3, 20, 10))
        ds = hv.Dataset(
            {"x": range(10), "y": range(20), "z": range(3), "Val": array, "Val2": array * 2},
            kdims=["x", "y", "z"],
            vdims=["Val", "Val2"],
        )
        with DatatypeContext([self.datatype, "dictionary", "dataframe"], (ds, hv.Dataset)):
    ...
    def test_dataset_groupby_drop_dims_dynamic_with_vdim(self, rng):
        array = rng.random((3, 20, 10))
        ds = hv.Dataset(
            {"x": range(10), "y": range(20), "z": range(3), "Val": array, "Val2": array * 2},
            kdims=["x", "y", "z"],
    ...
    def test_dataset_ndloc_lists(self):
        ...
        ds = self.element(
            (xs, ys, arr), kdims=["x", "y"], vdims=["z"], datatype=[self.datatype, "dictionary"]
        )
        sliced = self.element(
            (xs[[1, 2, 3]], ys[[0, 1, 2]], arr[[0, 1, 2], [1, 2, 3]]),
            kdims=["x", "y"],
            vdims=["z"],
            datatype=["dictionary"],
        )
        assert_element_equal(ds.ndloc[[0, 1, 2], [1, 2, 3]], sliced)
    ...
    def test_dataset_ndloc_lists_invert_x(self):
        ...
        ds = self.element(
            ...
            kdims=["x", "y"],
            vdims=["z"],
            datatype=[self.datatype, "dictionary"],
        )
        sliced = self.element(
            (xs[::-1][[8, 7, 6]], ys[[0, 1, 2]], arr[[0, 1, 2], [8, 7, 6]]),
            kdims=["x", "y"],
            vdims=["z"],
            datatype=["dictionary"],
        )
    ...
    def test_dataset_ndloc_lists_invert_y(self):
        ...
        sliced = self.element(
            (xs[[1, 2, 3]], ys[::-1][[4, 3, 2]], arr[[4, 3, 2], [1, 2, 3]]),
            kdims=["x", "y"],
            vdims=["z"],
            datatype=["dictionary"],
        )
    ...
    def test_dataset_ndloc_lists_invert_xy(self):
        ...
        sliced = self.element(
            (xs[::-1][[8, 7, 6]], ys[::-1][[4, 3, 2]], arr[[4, 3, 2], [8, 7, 6]]),
            kdims=["x", "y"],
            vdims=["z"],
            datatype=["dictionary"],
        )
    ...
    def test_dataset_ndloc_slice_two_vdims(self):
        ...
        sliced = self.element(
            (xs[[1, 2, 3]], ys[[0, 1, 2]], arr[[0, 1, 2], [1, 2, 3]], arr2[[0, 1, 2], [1, 2, 3]]),
            kdims=["x", "y"],
            vdims=["z", "z2"],
            datatype=["dictionary"],
        )
    ...
    def test_mask_2d_array(self, rng):
    ...
    def test_mask_2d_array_x_reversed(self, rng):
        array = rng.random((4, 3))
    ...
    def test_mask_2d_array_y_reversed(self, rng):
        array = rng.random((4, 3))
    ...
    def test_mask_2d_array_xy_reversed(self, rng):
        array = rng.random((4, 3))
...
class DaskGridInterfaceTests(GridInterfaceTests):
    ...
    def test_dataset_array_hm(self):
        assert_data_equal(
            self.dataset_hm.array(), np.column_stack([self.xs, self.y_ints.compute()])
        )

    def test_dataset_array_hm_alias(self):
        assert_data_equal(
            self.dataset_hm_alias.array(), np.column_stack([self.xs, self.y_ints.compute()])
        )
    ...
    def test_dataset_2D_columnar_shape(self, rng):
        array = da.from_array(rng.random((11, 11)), 3)
        dataset = hv.Dataset(
            {"x": self.xs, "y": self.y_ints, "z": array}, kdims=["x", "y"], vdims=["z"]
        )
        assert dataset.shape == (11 * 11, 3)
    ...
    def test_dataset_2D_gridded_shape(self, rng):
        array = da.from_array(rng.random((12, 11)), 3)
        dataset = hv.Dataset(
            {"x": self.xs, "y": range(12), "z": array}, kdims=["x", "y"], vdims=["z"]
        )
        assert dataset.interface.shape(dataset, gridded=True) == (12, 11)
    ...
    def test_dataset_2D_aggregate_partial_hm(self, rng):
        ...
        assert_element_equal(
            dataset.aggregate(["x"], np.mean),
            hv.Dataset(
                {"x": self.xs, "z": np.mean(array, axis=0).compute()}, kdims=["x"], vdims=["z"]
            ),
        )
    ...
    def test_dataset_2D_aggregate_partial_hm_alias(self, rng):
        ...
        assert_element_equal(
            dataset.aggregate(["X"], np.mean),
            hv.Dataset(
                {"x": self.xs, "z": np.mean(array, axis=0).compute()},
                kdims=[("x", "X")],
                vdims=[("z", "Z")],
            ),
        )
    ...
    def test_dataset_groupby_dynamic(self, rng):
        ...
        with DatatypeContext([self.datatype, "dictionary", "dataframe"], dataset):
            grouped = dataset.groupby("x", dynamic=True)
        first = hv.Dataset(
            {"y": self.y_ints.compute(), "z": array[:, 0]}, kdims=["y"], vdims=["z"]
        )
        assert_element_equal(grouped[0], first)
    ...
    def test_dataset_groupby_dynamic_alias(self, rng):
        ...
        with DatatypeContext([self.datatype, "dictionary", "dataframe"], dataset):
            grouped = dataset.groupby("X", dynamic=True)
        first = hv.Dataset(
            {"y": self.y_ints.compute(), "z": array[:, 0].compute()},
            kdims=[("y", "Y")],
            vdims=[("z", "Z")],
        )
        assert_element_equal(grouped[0], first)
    ...
    def test_dataset_groupby_multiple_dims(self, rng):
        dataset = hv.Dataset(
            (range(8), range(8), range(8), range(8), da.from_array(rng.random((8, 8, 8, 8)), 4)),
            kdims=["a", "b", "c", "d"],
            vdims=["Value"],
        )
        grouped = dataset.groupby(["c", "d"])
    ...
    def test_dataset_groupby_drop_dims(self, rng):
        array = da.from_array(rng.random((3, 20, 10)), 3)
        ds = hv.Dataset(
            {"x": range(10), "y": range(20), "z": range(3), "Val": array},
            kdims=["x", "y", "z"],
            vdims=["Val"],
...
class ImageElement_GridInterfaceTests(BaseImageElementInterfaceTests):
    ...
    def test_slice_datetime_xaxis(self):
        ...
        assert_data_equal(sliced.dimension_values(2, flat=False), self.array[:, 5:8])
    ...
    def test_slice_datetime_yaxis(self):
        ...
        assert_data_equal(sliced.dimension_values(2, flat=False), self.array[1:5, :])

    def test_slice_xaxis_inv(self):
        sliced = self.image_inv[0.3:5.2]
        assert sliced.bounds.lbrt() == (0, 0, 6, 10)
        assert sliced.xdensity == 0.5
        assert sliced.ydensity == 1
        assert_data_equal(sliced.dimension_values(2, flat=False), self.array[:, 5:8])

    def test_slice_yaxis_inv(self):
        sliced = self.image_inv[:, 1.2:5.2]
        assert sliced.bounds.lbrt() == (-10, 1.0, 10, 5)
        assert sliced.xdensity == 0.5
        assert sliced.ydensity == 1
        assert_data_equal(sliced.dimension_values(2, flat=False), self.array[1:5, :])
    ...
    def test_slice_both_axes_inv(self):
        ...
        assert sliced.bounds.lbrt() == (0, 1.0, 6, 5)
        assert sliced.xdensity == 0.5
        assert sliced.ydensity == 1
        assert_data_equal(sliced.dimension_values(2, flat=False), self.array[1:5, 5:8])

    def test_slice_x_index_y_inv(self):
        sliced = self.image_inv[0.3:5.2, 5.2]
        assert sliced.bounds.lbrt() == (0, 5.0, 6.0, 6.0)
        assert sliced.xdensity == 0.5
        assert sliced.ydensity == 1
        assert_data_equal(sliced.dimension_values(2, flat=False), self.array[5:6, 5:8])

    def test_index_x_slice_y_inv(self):
        sliced = self.image_inv[3.2, 1.2:5.2]
        assert sliced.bounds.lbrt() == (2.0, 1.0, 4.0, 5.0)
        assert sliced.xdensity == 0.5
        assert sliced.ydensity == 1
        assert_data_equal(sliced.dimension_values(2, flat=False), self.array[1:5, 6:7])
