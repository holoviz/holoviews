...
class Binned1DTest:
    ...
    def test_groupby_kdim(self):
        ...
        holomap = hv.HoloMap(
            {
                self.edges[i : i + 2].mean(): hv.Dataset([(i,)], vdims=["Frequency"])
                for i in range(10)
            },
            kdims=["x"],
        )
...
class Binned2DTest:
    ...
    def test_groupby_xdim(self):
        ...
        holomap = hv.HoloMap(
            {
                (self.xs[i] + np.diff(self.xs[i : i + 2]) / 2.0)[0]: hv.Dataset(
    ...
    def test_groupby_ydim(self):
        ...
        holomap = hv.HoloMap(
            {
                self.ys[i : i + 2].mean(): hv.Dataset((self.xs, self.zs[i]), "x", "z")
                for i in range(3)
            },
            kdims=["y"],
        )
...
class Irregular2DBinsTest:
    ...
    def test_construct_3d_from_xarray(self):
        ...
        da = xr.DataArray(
            zs,
            dims=["z", "y", "x"],
            coords={"lat": (("y", "x"), self.ys), "lon": (("y", "x"), self.xs), "z": [0, 1]},
            name="A",
        )
    ...
    def test_construct_from_xarray_with_invalid_irregular_coordinate_arrays(self):
        ...
        da = xr.DataArray(
            zs,
            dims=["z", "y", "x", "b"],
            coords={"lat": (("y", "b"), self.ys), "lon": (("y", "x"), self.xs), "z": [0, 1]},
            name="A",
        )
    ...
    def test_3d_xarray_with_constant_dim_canonicalized_to_2d(self):
        ...
        da = xr.DataArray(
            zs,
            dims=["z", "y", "x"],
            coords={"lat": (("y", "x"), self.ys), "lon": (("y", "x"), self.xs), "z": [0]},
            name="A",
        )
    ...
    def test_groupby_3d_from_xarray(self):
        ...
        hmap = hv.HoloMap(
            {
                0: hv.Dataset((self.xs, self.ys, zs[0]), ["lon", "lat"], "A"),
                1: hv.Dataset((self.xs, self.ys, zs[1]), ["lon", "lat"], "A"),
            },
            kdims="z",
        )
    ...
    def test_irregular_transform_replace_kdim(self):
        transformed = hv.Dataset((self.xs, self.ys, self.zs), ["x", "y"], "z").transform(
            x=hv.dim("x") * 2
        )
