...
class TestLabelsPlot(TestMPLPlot):
    def test_labels_simple(self):
        ...
        for i, text in enumerate(artist):
            assert text._x == expected["x"][i]
            assert text._y == expected["y"][i]
    ...
    def test_labels_inverted(self):
        ...
        expected = {"x": np.array([0, 1]), "y": np.array([1, 0]), "Label": ["A", "B"]}
        for i, text in enumerate(artist):
            assert text._x == expected["y"][i]
            assert text._y == expected["x"][i]
            assert text.get_text() == expected["Label"][i]
    ...
    def test_labels_color_mapped(self):
        ...
        plot = mpl_renderer.get_plot(labels)
        artist = plot.handles["artist"]
        expected = {"x": np.array([0, 1]), "y": np.array([1, 0]), "Label": ["0.33333", "0.66666"]}
        colors = [
            (0.26666666666666666, 0.0039215686274509803, 0.32941176470588235, 1.0),
        ...
        labels = hv.HoloMap(
            {
                ...
                1: hv.Labels(
                    [(0, 0, "#FF0000"), (0, 1, "#00FF00"), (0, 2, "#0000FF")], vdims="color"
                ),
            }
        ).opts(color="color")
        plot = mpl_renderer.get_plot(labels)
    ...
    def test_label_linear_color_op(self):
        labels = hv.Labels([(0, 0, 0), (0, 1, 1), (0, 2, 2)], vdims="color").opts(color="color")
        plot = mpl_renderer.get_plot(labels)
        artist = plot.handles["artist"]
        assert [rgb2hex(a.get_color()) for a in artist] == ["#440154", "#20908c", "#fde724"]

    def test_label_categorical_color_op(self):
        labels = hv.Labels([(0, 0, "A"), (0, 1, "B"), (0, 2, "A")], vdims="color").opts(
            color="color", cmap="tab10"
        )
        plot = mpl_renderer.get_plot(labels)
        artist = plot.handles["artist"]
    ...
    def test_label_size_op(self):
        labels = hv.Labels([(0, 0, 8), (0, 1, 12), (0, 2, 6)], vdims="size").opts(size="size")
        plot = mpl_renderer.get_plot(labels)
        artist = plot.handles["artist"]
    ...
    def test_label_size_op_update(self):
        labels = hv.HoloMap(
            {
                0: hv.Labels([(0, 0, 8), (0, 1, 6), (0, 2, 12)], vdims="size"),
                1: hv.Labels([(0, 0, 9), (0, 1, 4), (0, 2, 3)], vdims="size"),
            }
        ).opts(size="size")
        plot = mpl_renderer.get_plot(labels)
        artist = plot.handles["artist"]
        assert [a.get_fontsize() for a in artist] == [8, 6, 12]
        plot.update((1,))
        artist = plot.handles["artist"]
    ...
    def test_label_alpha_op_update(self):
        labels = hv.HoloMap(
            {
                0: hv.Labels([(0, 0, 0.3), (0, 1, 1), (0, 2, 0.6)], vdims="alpha"),
                1: hv.Labels([(0, 0, 0.6), (0, 1, 0.1), (0, 2, 1)], vdims="alpha"),
            }
        ).opts(alpha="alpha")
        plot = mpl_renderer.get_plot(labels)
        artist = plot.handles["artist"]
        assert [a.get_alpha() for a in artist] == [0.3, 1, 0.6]
        plot.update((1,))
        artist = plot.handles["artist"]
        assert [a.get_alpha() for a in artist] == [0.6, 0.1, 1]

    def test_label_rotation_op(self):
        labels = hv.Labels([(0, 0, 90), (0, 1, 180), (0, 2, 270)], vdims="rotation").opts(
            rotation="rotation"
        )
        plot = mpl_renderer.get_plot(labels)
        artist = plot.handles["artist"]
    ...
    def test_label_rotation_op_update(self):
        labels = hv.HoloMap(
            {
                0: hv.Labels([(0, 0, 45), (0, 1, 180), (0, 2, 90)], vdims="rotation"),
                1: hv.Labels([(0, 0, 30), (0, 1, 120), (0, 2, 60)], vdims="rotation"),
            }
        ).opts(rotation="rotation")
        plot = mpl_renderer.get_plot(labels)
        artist = plot.handles["artist"]
        assert [a.get_rotation() for a in artist] == [45, 180, 90]
        plot.update((1,))
        artist = plot.handles["artist"]
