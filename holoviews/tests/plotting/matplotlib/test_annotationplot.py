...
class TestHVLinesPlot(TestMPLPlot):
    def test_hlines_plot(self):
        ...
        for source, val in zip(sources, hlines.data["y"], strict=True):
    ...
    def test_hlines_array(self):
        ...
        sources = plot.handles["annotations"]
        assert len(sources) == 4
        for source, val in zip(sources, hlines.data, strict=True):
            assert source.get_data() == ([0, 1], [val, val])
    ...
    def test_hlines_nondefault_kdim(self):
        ...
        sources = plot.handles["annotations"]
        assert len(sources) == 4
        for source, val in zip(sources, hlines.data["other"], strict=True):
            assert source.get_data() == ([0, 1], [val, val])
    ...
    def test_vlines_plot(self):
        ...
        sources = plot.handles["annotations"]
        assert len(sources) == 4
        for source, val in zip(sources, vlines.data["x"], strict=True):
            assert source.get_data() == ([val, val], [0, 1])
    ...
    def test_vlines_nondefault_kdim(self):
        ...
        sources = plot.handles["annotations"]
        assert len(sources) == 4
        for source, val in zip(sources, vlines.data["other"], strict=True):
            assert source.get_data() == ([val, val], [0, 1])
    ...
    def test_vlines_hlines_overlay(self):
        ...
        for source, val in zip(sources[:4], hlines.data["y"], strict=True):
            assert source.get_data() == ([0, 1], [val, val])

        for source, val in zip(sources[4:8], vlines.data["x"], strict=True):
            assert source.get_data() == ([val, val], [0, 1])
...
class TestHVSpansPlot(TestMPLPlot):
    ...
    def test_hspans_plot(self):
        ...
        for source, v0, v1 in zip(sources, hspans.data["y0"], hspans.data["y1"], strict=True):
    ...
    def test_vspans_plot(self):
        ...
        sources = plot.handles["annotations"]
        assert len(sources) == 3
        for source, v0, v1 in zip(sources, vspans.data["x0"], vspans.data["x1"], strict=True):
            self._vspans_check(source, v0, v1)
    ...
    def test_vspans_hspans_overlay(self):
        ...
        sources = plot.handles["fig"].axes[0].get_children()
        for source, v0, v1 in zip(sources[:3], hspans.data["y0"], hspans.data["y1"], strict=True):
            self._hspans_check(source, v0, v1)

        for source, v0, v1 in zip(sources[3:6], vspans.data["x0"], vspans.data["x1"], strict=True):
            self._vspans_check(source, v0, v1)
