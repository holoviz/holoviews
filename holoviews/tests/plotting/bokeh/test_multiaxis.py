...
class TestCurveTwinAxes(LoggingComparison, TestBokehPlot):
    ...
    def test_multi_y_enabled_two_curves_two_vdim(self):
        overlay = (hv.Curve(range(10), vdims=["A"]) * hv.Curve(range(10), vdims=["B"])).opts(
            multi_y=True
        )
    ...
    def test_multi_y_lims_left_axis(self):
        overlay = (
            hv.Curve(range(10), vdims=["A"]).opts(ylim=(-10, 20))
    ...
    def test_multi_y_lims_right_axis(self):
        overlay = (
            ...
            * hv.Curve(range(10), vdims=["B"]).opts(ylim=(-10, 20))
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        y_range = plot.handles["y_range"]
        assert y_range.start == 0
    ...
    def test_multi_y_lims_both_axes(self):
        overlay = (
            hv.Curve(range(10), vdims=["A"]).opts(ylim=(-15, 25))
            * hv.Curve(range(10), vdims=["B"]).opts(ylim=(-10, 20))
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        y_range = plot.handles["y_range"]
    ...
    def test_multi_log_left_axis(self):
        overlay = (
            hv.Curve(range(1, 9), vdims=["A"]).opts(logy=True) * hv.Curve(range(10), vdims=["B"])
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        assert len(plot.state.yaxis) == 2
    ...
    def test_multi_log_right_axis(self):
        overlay = (
            hv.Curve(range(1, 9), vdims=["A"]) * hv.Curve(range(1, 9), vdims=["B"]).opts(logy=True)
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        assert len(plot.state.yaxis) == 2
    ...
    def test_multi_log_both_axes(self):
        overlay = (
            hv.Curve(range(1, 9), vdims=["A"]).opts(logy=True)
            * hv.Curve(range(1, 9), vdims=["B"]).opts(logy=True)
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        assert len(plot.state.yaxis) == 2
        assert isinstance(plot.state.yaxis[0], LogAxis)
    ...
    def test_multi_log_right_axis_warn(self):
        overlay = (
            hv.Curve(range(10), vdims=["A"]) * hv.Curve(range(10), vdims=["B"]).opts(logy=True)
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        assert len(plot.state.yaxis) == 2
        assert isinstance(plot.state.yaxis[0], LinearAxis)
    ...
    def test_multi_invert_left_axis(self):
        overlay = (
            hv.Curve(range(10), vdims=["A"]).opts(invert_yaxis=True)
            * hv.Curve(range(10), vdims=["B"])
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        y_range = plot.handles["y_range"]
    ...
    def test_multi_invert_right_axis(self):
        overlay = (
            hv.Curve(range(10), vdims=["A"])
            * hv.Curve(range(10), vdims=["B"]).opts(invert_yaxis=True)
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        y_range = plot.handles["y_range"]
        assert y_range.start == 0
    ...
    def test_inverted_log_ylim_right_axis(self):
        overlay = (
            hv.Curve(range(10), vdims=["A"])
            * hv.Curve(range(10), vdims=["B"]).opts(invert_yaxis=True, logy=True, ylim=(2, 20))
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        y_range = plot.handles["y_range"]
        assert y_range.start == 0
    ...
    def test_invisible_main_axis(self):
        overlay = (
            hv.Curve(range(10), vdims=["A"]).opts(yaxis=None) * hv.Curve(range(10), vdims=["B"])
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        assert len(plot.state.yaxis) == 2
    ...
    def test_invisible_extra_axis(self):
        overlay = (
            hv.Curve(range(10), vdims=["A"]) * hv.Curve(range(10), vdims=["B"]).opts(yaxis=None)
        ).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        assert len(plot.state.yaxis) == 2
    ...
    def test_swapped_position_custom_y_labels(self):
        overlay = (
            hv.Curve(range(10), vdims=["A"]).opts(yaxis="right", ylabel="A-custom")
