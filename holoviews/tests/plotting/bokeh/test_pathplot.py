...
class TestPathPlot(TestBokehPlot):
    def test_batched_path_line_color_and_color(self):
        ...
        overlay = hv.NdOverlay({i: hv.Path([[(i, j) for j in range(2)]]) for i in range(2)}).opts(
            opts
        )
    ...
    def test_batched_path_alpha_and_color(self):
        opts = {"NdOverlay": dict(legend_limit=0), "Path": dict(alpha=hv.Cycle(values=[0.5, 1]))}
        overlay = hv.NdOverlay({i: hv.Path([[(i, j) for j in range(2)]]) for i in range(2)}).opts(
            opts
        )
        plot = bokeh_renderer.get_plot(overlay).subplots[()]
    ...
    def test_path_continuously_varying_line_width_op(self):
        ...
        source = plot.handles["source"]
        np.testing.assert_array_equal(
            source.data["xs"], np.array([np.array([1, 2]), np.array([2, 3]), np.array([3, 4])])
        )
        np.testing.assert_array_equal(
            source.data["ys"], np.array([np.array([4, 3]), np.array([3, 2]), np.array([2, 1])])
        )
    ...
    def test_path_continuously_varying_color_legend(self):
        ...
        path = hv.Path(data, vdims="cat").opts(
            color="cat",
            cmap=dict(zip(levels, colors, strict=True)),
            line_width=4,
            show_legend=True,
        )
        plot = bokeh_renderer.get_plot(path)
    ...
    def test_path_multiple_segments_with_single_vdim(self):
        ...
        np.testing.assert_equal(
            source.data["xs"],
            [np.array([0, 1]), np.array([1, 1]), np.array([0, 0]), np.array([0, 1])],
        )
        np.testing.assert_equal(
            source.data["ys"],
            [np.array([0, 0]), np.array([0, 1]), np.array([0, 1]), np.array([1, 1])],
        )
    ...
    def test_path_style_mapped_scalar_segments_lengths_match(self):
        ...
        n_pts = 3
        data = [
            {"x": np.arange(n_pts) + x, "y": np.arange(n_pts) + 1, "c": x * 10} for x in range(5)
        ]
    ...
    def test_path_style_mapped_per_vertex_segments_lengths_match(self):
        ...
        n_pts = 7
        data = [
            {"x": np.arange(n_pts) + x, "y": np.arange(n_pts) + 1, "c": np.full(n_pts, x * 10)}
            for x in range(5)
        ]
        path = hv.Path(data, vdims=["c"]).opts(color="c", cmap="Turbo", colorbar=True)
        plot = bokeh_renderer.get_plot(path)
...
class TestPolygonPlot(TestBokehPlot):
    ...
    def test_polygons_colored(self):
        polygons = hv.NdOverlay(
            {j: hv.Polygons([[(i**j, i, j) for i in range(10)]], vdims="Value") for j in range(5)}
        )
    ...
    def test_polygons_colored_batched_unsanitized(self):
        polygons = hv.NdOverlay(
            {
                j: hv.Polygons(
                    [[(i**j, i, j) for i in range(10)] for i in range(2)],
        ...
        assert cmapper.high == 4
        source = plot.handles["source"]
        assert source.data["some_question_mark_unescaped_name"] == [
    ...
    def test_polygons_hover_color_op(self):
        polygons = hv.Polygons(
            ...
            vdims="color",
        ).opts(fill_color="color", tools=["hover"])
        plot = bokeh_renderer.get_plot(polygons)
    ...
    def test_polygons_holes_initialize(self):
        ...
        poly = hv.HoloMap(
            {
                0: hv.Polygons([{"x": xs, "y": ys, "holes": holes}]),
...
class TestContoursPlot(TestBokehPlot):
    ...
    def test_contours_empty_path(self):
        contours = hv.Contours(
            ...
            vdims=["color", "line_width"],
        ).opts(color="color", line_width="line_width")
        plot = bokeh_renderer.get_plot(contours)
        glyph = plot.handles["glyph"]
...
class TestDendrogramPlot(TestBokehPlot):
    ...
    def test_adjoint_plot_in_layout(self):
        ...
        msg = (
            "Adjoined dendrogram in a Layout, does not currently support `.opts(shared_axes=True)`"
        )
