...
class TestRasterPlot(TestBokehPlot):
    ...
    def test_image_stack_tuple_unspecified_dims():
        x = np.arange(0, 3)
        y = np.arange(5, 8)
        a = np.array([[np.nan, np.nan, 1], [np.nan] * 3, [np.nan] * 3])
        b = np.array([[np.nan] * 3, [1, 1, np.nan], [np.nan] * 3])
    ...
    def test_image_stack_dict_unspecified_dims():
        x = np.arange(0, 3)
        y = np.arange(5, 8)
        a = np.array([[np.nan, np.nan, 1], [np.nan] * 3, [np.nan] * 3])
        b = np.array([[np.nan] * 3, [1, 1, np.nan], [np.nan] * 3])
    ...
    def test_image_stack_xarray_dataset():
        try:
            import xarray as xr
        except ImportError:
    ...
    def test_image_stack_xarray_dataarray():
        try:
            import xarray as xr
        except ImportError:
            raise SkipTest("xarray not available for core tests")
    ...
    def test_image_stack_invert_xaxis():
        x = np.arange(0, 3)
        y = np.arange(5, 8)
        a = np.array([[np.nan, np.nan, 1], [np.nan] * 3, [np.nan] * 3])
        b = np.array([[np.nan] * 3, [1, 1, np.nan], [np.nan] * 3])
    ...
    def test_image_stack_invert_yaxis():
        x = np.arange(0, 3)
        y = np.arange(5, 8)
        a = np.array([[np.nan, np.nan, 1], [np.nan] * 3, [np.nan] * 3])
        b = np.array([[np.nan] * 3, [1, 1, np.nan], [np.nan] * 3])
    ...
    def test_image_stack_invert_axes():
        x = np.arange(0, 3)
        y = np.arange(5, 8)
        a = np.array([[np.nan, np.nan, 1], [np.nan] * 3, [np.nan] * 3])
        b = np.array([[np.nan] * 3, [1, 1, np.nan], [np.nan] * 3])
