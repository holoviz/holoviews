...
class TestCurvePlot(TestBokehPlot):
    def test_batched_curve_subscribers_correctly_attached(self):
        ...
        overlay = hv.DynamicMap(
            lambda x: hv.NdOverlay(
                {i: hv.Curve([(i, j) for j in range(2)]) for i in range(2)}
            ).opts(opts),
            kdims=[],
            streams=[posx],
        )
    ...
    def test_cyclic_palette_curves(self):
        ...
        for subp, color in zip(plot.subplots.values(), colors, strict=True):
    ...
    def test_batched_curve_line_color_and_color(self):
        ...
        overlay = hv.NdOverlay({i: hv.Curve([(i, j) for j in range(2)]) for i in range(2)}).opts(
            opts
        )
    ...
    def test_batched_curve_alpha_and_color(self):
        opts = {"NdOverlay": dict(legend_limit=0), "Curve": dict(alpha=hv.Cycle(values=[0.5, 1]))}
        overlay = hv.NdOverlay({i: hv.Curve([(i, j) for j in range(2)]) for i in range(2)}).opts(
            opts
        )
        plot = bokeh_renderer.get_plot(overlay).subplots[()]
    ...
    def test_curve_overlay_datetime_hover(self):
        obj = hv.NdOverlay(
            {i: hv.Curve([(dt.datetime(2016, 1, j + 1), j) for j in range(31)]) for i in range(5)},
            kdims=["Test"],
        )
    ...
    def test_curve_hover_dimension(self):
        obj = hv.Curve(range(10), hv.Dimension("name", label="label")).opts(
            hover_tooltips=["label"]
        )
        self._test_hover_info(obj, [("label", "@{name}")], "nearest")

    def test_curve_hover_dimension_unit(self):
        obj = hv.Curve(range(10), hv.Dimension("name", label="label", unit="unit")).opts(
            hover_tooltips=["label"]
        )
    ...
    def test_curve_scalar_color_op(self):
        curve = hv.Curve([(0, 0, "red"), (0, 1, "red"), (0, 2, "red")], vdims=["y", "color"]).opts(
            color="color"
        )
        plot = bokeh_renderer.get_plot(curve)
    ...
    def test_op_ndoverlay_color_value(self):
        ...
        overlay = hv.NdOverlay(
            {color: hv.Curve(np.arange(i)) for i, color in enumerate(colors)}, "color"
    ...
    def test_curve_color_op(self):
        curve = hv.Curve(
            [(0, 0, "red"), (0, 1, "blue"), (0, 2, "red")], vdims=["y", "color"]
        ).opts(color="color")
    ...
    def test_curve_alpha_op(self):
        curve = hv.Curve([(0, 0, 0.1), (0, 1, 0.3), (0, 2, 1)], vdims=["y", "alpha"]).opts(
            alpha="alpha"
        )
    ...
    def test_curve_line_width_op(self):
        curve = hv.Curve([(0, 0, 0.1), (0, 1, 0.3), (0, 2, 1)], vdims=["y", "linewidth"]).opts(
