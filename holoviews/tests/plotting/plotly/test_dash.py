...
class TestHoloViewsDash(TestPlotlyPlot):
    ...
    def test_boundsxy_dynamic_map(self):
        ...
        # Check inputs
        expected_inputs = [
            (g.id, prop) for g in components.graphs for prop in ["selectedData", "relayoutData"]
        ...
        with patch.object(
            CallbackContext, "triggered", [{"prop_id": inputs[0].component_id + ".selectedData"}]
        ):
            [fig1, fig2, new_store] = callback_fn(
                {"range": {"x": [1, 2], "y": [3, 4]}}, {}, {}, {}, 0, store_value
            )
        ...
        assert decode_store_data(new_store) == {
        ...
        # Click reset button
        with patch.object(
            CallbackContext, "triggered", [{"prop_id": components.resets[0].id + ".n_clicks"}]
        ):
            [fig1, fig2, new_store] = callback_fn(
                {"range": {"x": [1, 2], "y": [3, 4]}}, {}, {}, {}, 1, store_value
            )
    ...
    def test_rangexy_dynamic_map(self):
        ...
        scatter = hv.Scatter([[0, 1], [0, 1]], kdims=["x"], vdims=["y"])
        ...
        def dmap_fn(x_range, y_range):
            ...
            return hv.Scatter(
                [[x_range[0], y_range[0]], [x_range[1], y_range[1]]], kdims=["x1"], vdims=["y1"]
            )
        ...
        with patch.object(
            CallbackContext, "triggered", [{"prop_id": components.graphs[0].id + ".relayoutData"}]
        ):
            [fig1, fig2, new_store] = callback_fn(
                {},
                {
    ...
    def test_selection1d_dynamic_map(self):
        ...
        dmap = hv.DynamicMap(
            lambda index: scatter.iloc[index].opts(size=len(index) + 1), streams=[selection1d]
        )
        ...
        with patch.object(
            ...
            [_fig1, fig2, new_store] = callback_fn({}, {}, 1, store)
        ...
        assert_element_equal(
            ...
            {"streams": {id(selection1d): {"index": []}}, "reset_nclicks": 1, "kdims": {}},
        )

    def test_kdims_dynamic_map(self):
        # Dynamic map with two key dimensions
        dmap = hv.DynamicMap(
            lambda kdim1: hv.Scatter([kdim1, kdim1]), kdims=["kdim1"]
        ...
        with patch.object(CallbackContext, "triggered", []):
            [fig, _new_store] = callback_fn({}, {}, 3, None, store_value)
