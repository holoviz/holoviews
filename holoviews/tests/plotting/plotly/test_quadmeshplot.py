...
class TestQuadMeshPlot(TestPlotlyPlot):
    def test_quadmesh_state(self):
        ...
        assert_data_equal(state["data"][0]["x"], np.array([0.5, 1.5, 3.0, 5.0]))
        assert_data_equal(state["data"][0]["y"], np.array([-0.5, 0.5, 1.5]))
    ...
    def test_quadmesh_nodata(self):
        img = hv.QuadMesh(([1, 2, 4], [0, 1], np.array([[0, 1, 2], [2, 3, 4]]))).opts(nodata=0)
        state = self._get_plot_state(img)
        assert state["data"][0]["type"] == "heatmap"
    ...
    def test_quadmesh_nodata_uint(self):
        img = hv.QuadMesh(
            ([1, 2, 4], [0, 1], np.array([[0, 1, 2], [2, 3, 4]], dtype="uint32"))
        ).opts(nodata=0)
        state = self._get_plot_state(img)
        assert state["data"][0]["type"] == "heatmap"
        assert_data_equal(state["data"][0]["z"], np.array([[np.nan, 1, 2], [2, 3, 4]]))
    ...
    def test_quadmesh_state_inverted(self):
        ...
        state = self._get_plot_state(img)
        assert_data_equal(state["data"][0]["x"], np.array([-0.5, 0.5, 1.5]))
        assert_data_equal(state["data"][0]["y"], np.array([0.5, 1.5, 3.0, 5.0]))
    ...
    def test_visible(self):
        element = hv.QuadMesh(([1, 2, 4], [0, 1], np.array([[0, 1, 2], [2, 3, 4]]))).opts(
            visible=False
        )
