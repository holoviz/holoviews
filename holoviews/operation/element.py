...
class gradient(Operation):
    ...
    def _process(self, matrix, key=None):
        ...
        if len(matrix.vdims) != 1:
            raise ValueError(
                "Input matrix to gradient operation must have single value dimension."
            )
    ...
    def _process(self, element, key=None):
        ...
        if any(data_is_datetime):
            if any(data_is_datetime[:2]) and self.p.filled:
                raise RuntimeError(
                    "Datetime spatial coordinates are not supported "
            ...
            try:
                ...
                raise ImportError(
                    "contours operation using datetimes requires matplotlib."
                ) from None
        ...
        if self.p.filled:
            ...
            for lower_level, upper_level in pairwise(levels):
                ...
                for jstart, jend in pairwise(outer_offsets):
                    ...
                    interior = [
                        points[offsets[j] : offsets[j + 1]] for j in range(jstart + 1, jend)
                    ]
                    interiors.append(interior)
                level = (lower_level + upper_level) / 2
                geom = {
                    element.vdims[0].name: num2date(level) if data_is_datetime[2] else level,
    ...
    def _process(self, element, key=None, groupby=False):
        ...
        if isinstance(self.p.bins, str):
            ...
            if isinstance(start, str) or isinstance(end, str) or isinstance(steps, str):
                raise ValueError(
                    "Categorical data found. Cannot create histogram from categorical data."
                )
            if is_datetime:
...
class categorical_agg(Operation):
    ...
    dimension = param.String(default=None, allow_None=True, doc="""
        Categorical dimension to group by.""")

    value_dimension = param.String(default=None, allow_None=True, doc="""
        Numeric dimension to aggregate. If None, counts rows per category.""")

    function = param.Callable(default=np.size, doc="""
        Aggregation function applied to value_dimension values.""")

    label = param.String(default=None, allow_None=True, doc="""
        Label for the value axis. Auto-generated from function name if None.""")

    def _process(self, element, key=None):
        if self.p.dimension is None:
            raise ValueError(
                "The dimension parameter is required for categorical_agg."
            )

        cat_dim = element.get_dimension(self.p.dimension)
        if cat_dim is None:
            raise ValueError(
                f"Dimension '{self.p.dimension}' not found on the input element."
            )
        ...
        if self.p.value_dimension is None:
            ...
            if val_dim is None:
                raise ValueError(
                    f"Value dimension '{self.p.value_dimension}' not found "
                    f"on the input element."
                )
            num_vals = element.dimension_values(
                self.p.value_dimension, expanded=True
            )
            results = []
            ...
            if d1 == d2:
                if p.diagonal_type is not None:
                    if p.diagonal_type._auto_indexable_1d:
                        el = p.diagonal_type(
                            el_data, kdims=[d1], vdims=[d2], datatype=[default_datatype]
                        )
                    else:
...
class dendrogram(Operation):
    ...
    def _process(self, element, key=None):
        ...
        if type(element) is not Dataset:
            ...
            main = self.p.main_element(
                dataset.sort(sort_dims).reindex(element_kdims[:2]), vdims=vdims
            )
