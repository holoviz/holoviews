...
class _connect_edges(Operation):
    ...
    def _bundle(self, position_df, edges_df):
        raise NotImplementedError(
            "_connect_edges is an abstract baseclass and does not implement any actual bundling."
        )

    def _process(self, element, key=None):
        index = element.nodes.kdims[2].name
        rename_edges = {
            d.name: v for d, v in zip(element.kdims[:2], ["source", "target"], strict=True)
        }
        rename_nodes = {
            d.name: v for d, v in zip(element.nodes.kdims[:2], ["x", "y"], strict=True)
        }
