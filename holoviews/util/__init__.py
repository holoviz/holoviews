...
class opts(param.ParameterizedFunction, metaclass=OptsMeta):
    ...
    def _group_kwargs_to_options(cls, obj, kwargs):
        ...
        if any(targets) and not all(targets):
            ...
            if obj.label:
                identifier = (
                    f"{obj.__class__.__name__}.{sanitized_group}.{util.label_sanitizer(obj.label)}"
                )
    ...
    def _expand_by_backend(cls, options, backend):
        ...
        if backend and not used_fallback:
            cls.param.warning(
                "All supplied Options objects already define a backend, "
    ...
    def _expand_options(cls, options, backend=None):
        ...
        if not Store.renderers:
            raise ValueError(
                "No plotting extension is currently loaded. "
            ...
            raise ValueError(
                f"Currently selected plotting extension {current_backend!r} "
...
class output(param.ParameterizedFunction):
    ...
    def __call__(self, *args, **options):
        ...
        if isinstance(obj, Dimensioned):
            ...
            Store.output_settings.output(
                line=line, cell=obj, cell_runner=display_fn, help_prompt=help_prompt, **options
            )
...
class extension(_pyviz_extension):
        ...
        for backend, imp in imports:
            ...
            for hook in self._backend_hooks[backend]:
                try:
                    ...
                    self.param.warning(
                        f"{backend} backend hook {hook} failed with following exception: {e}"
                    )
    ...
    def _ignore_bokeh_warnings(self):
        ...
        warnings.filterwarnings(
            "ignore", category=BokehUserWarning, message="reference already known"
        )
...
class Dynamic(param.ParameterizedFunction):
    ...
    def __call__(self, map_obj, **params):
        ...
        if isinstance(map_obj, DynamicMap):
            kwargs = dict(shared_data=self.p.shared_data, callback=callback, streams=streams)
    ...
    def _get_streams(self, map_obj, watch=True):
        ...
        if isinstance(self.p.streams, dict):
            ...
            for name, p in self.p.streams.items():
                if not isinstance(p, param.Parameter):
                    raise ValueError(
                        "Stream dictionary must map operation keywords "
        ...
        for stream in stream_specs:
            if inspect.isclass(stream) and issubclass(stream, Stream):
                ...
                raise ValueError(
                    f"Streams must be Stream classes or instances, found {type(stream).__name__} type"
                )
            if isinstance(op, Operation):
                updates = {
                    k: op.p.get(k) for k, v in stream.contents.items() if v is None and k in op.p
                }
        ...
        has_dependencies = (
            util.is_param_method(op, has_deps=True)
        ...
        for value in self.p.kwargs.values():
            if util.is_param_method(value, has_deps=True):
                ...
                params = [
                    ...
                    if isinstance(d, param.Parameter) and isinstance(d.owner, param.Parameterized)
                ]
    ...
    def _process(self, element, key=None, kwargs=None):
        if kwargs is None:
            kwargs = {}
        if (
            util.is_param_method(self.p.operation)
            and util.get_method_owner(self.p.operation) is element
        ):
