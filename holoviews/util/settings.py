...
def list_backends():
    ...
    for backend in Store.renderers:
        ...
        modes = [
            mode
            for mode in renderer.param.objects("existing")["mode"].objects
            if mode != "default"
        ]
...
class OutputSettings(KeywordSettings):
    ...
    def missing_backend_exception(value, keyword, allowed):
        if value in OutputSettings.backend_list:
            raise ValueError(
                f"Backend {value!r} not available. Has it been loaded with the notebook_extension?"
            )
        else:
            raise ValueError(f"Backend {value!r} does not exist")

    custom_exceptions = {
        "holomap": missing_dependency_exception,
    ...
    def _generate_docstring(cls, signature=False):
        ...
        max_frames = "max_frames   : The max number of frames rendered (default {!r})".format(
            cls.defaults["max_frames"]
        )
        size = "size         : The percentage size of displayed output"
        dpi = "dpi          : The rendered dpi of the figure"
        filename = "filename    : The filename of the saved output, if any (default {!r})".format(
            cls.defaults["filename"]
        )
        info = (
            "info    : The information to page about the displayed objects (default {!r})".format(
                cls.defaults["info"]
            )
        )
    ...
    def _validate(cls, options, items, warnfn):
        ...
        if "html" in Store.display_formats:
            ...
            msg = (
                f"Requesting output figure format {items['fig']!r} "
    ...
    @classmethod
    def output(
        cls, line=None, cell=None, cell_runner=None, help_prompt=None, warnfn=None, **kwargs
    ):
        ...
        if prev_backend in Store.renderers:
            ...
            prev_params = {
                k: v for k, v in prev_renderer.param.values().items() if k in cls.render_params
            }
        else:
            prev_renderer = None
        ...
        try:
            ...
            renderer = Store.renderers[backend]
            render_params = {
                k: v for k, v in renderer.param.values().items() if k in cls.render_params
            }
            ...
            if backend not in Store.renderers:
                raise ValueError(
                    f"The selected plotting extension {backend!r} "
    ...
    def update_options(cls, options, items):
        ...
        cls._backend_options[prev_backend_spec] = {
            k: v for k, v in cls.options.items() if k in cls.remembered
        }
