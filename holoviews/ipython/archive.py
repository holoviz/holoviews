...
class NotebookArchive(FileArchive):
    ...
    skip_notebook_export = param.Boolean(
        default=False,
        doc="""
    ...
    def auto(self, enabled=True, clear=False, **kwargs):
        ...
        print(
            "Automatic capture is now {}.{}".format(
                "enabled" if enabled else "disabled", tstamp if enabled else ""
            )
        )
    ...
    def export(self, timestamp=None):
        ...
        capture_cmd = (
            rf"var capture = '{name}._notebook_data=r\"\"\"'"
        ) + r"+json_string+'\"\"\"'; "
        cmd = (
            r"var kernel = IPython.notebook.kernel; "
        ...
        export_name = self._format(
            self.export_name, {"timestamp": tstamp, "notebook": self.notebook_name}
        )
        print(
            (
                f"Export name: {export_name!r}\nDirectory    {os.path.join(os.path.abspath(self.root))!r}"
            )
    ...
    def _export_with_html(self):  # pragma: no cover
        ...
        try:
            ...
            for (basename, ext), entry in self._files.items():
                ...
                if html_key is None:
                    continue
                filename = self._format(
                    basename, {"timestamp": tstamp, "notebook": self.notebook_name}
                )
                ...
                # Not displayable in an HTML tag
                    ...
                    link_html = self._format(
                        self._tags[info["mime_type"]],
                        {"src": truncated, "mime_type": info["mime_type"], "css": ""},
                    )
