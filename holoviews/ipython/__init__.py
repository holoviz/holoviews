...
class notebook_extension(extension):
    ...
    def __call__(self, *args, **params):
        ...
        Renderer.load_nb(
            inline=p.inline, reloading=same_cell_execution, enable_mathjax=p.enable_mathjax
        )
        ...
        if not same_cell_execution and p.logo:
            self.load_logo(
                ...
                mpl_logo=p.logo and (("matplotlib" in resources) or resources == ["holoviews"]),
    ...
    def _get_resources(self, args, params):
        ...
        if unmatched_args:
            display(
                HTML(
                    "<b>Warning:</b> Unrecognized resources '{}'".format(
                        "', '".join(unmatched_args)
                    )
                )
            )
    ...
    @classmethod
    def load_logo(
        cls, logo: dict | bool = False, bokeh_logo=False, mpl_logo=False, plotly_logo=False
    ):
        ...
        hide_tooltip = os.getenv("HV_HIDE_TOOLTIP_LOGO", os.getenv("CI", "0")).lower() in (
            "1",
            "true",
        )
