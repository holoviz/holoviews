*******************
holoviews Tutorials
*******************

The best way to learn what Holoviews can do and how to use it is to
study the HoloViews tutorials:

* `Homepage: <http://nbviewer.ipython.org/url/homepages.inf.ed.ac.uk/jbednar/holoviews/Homepage.ipynb>`_
  Images to go on the HoloViews home page.

* `Showcase: <http://ioam.github.io/holoviews/Tutorials/Showcase.html>`_
  What HoloViews can do for you and your data.

* `Introduction: <http://ioam.github.io/holoviews/Tutorials/Introduction.html>`_
  How to use HoloViews -- basic concepts and getting started.

* `Exploring Data: <http://http://ioam.github.io/holoviews/Tutorials/Exploring_Data.html>`_
  Introduction to the data structure that flexibly holds all your data
  ready for selecting, sampling, slicing, viewing, and
  combining.

* `Transforming Data: <http://ioam.github.com/media/holoviews/Transforming_Data.html>`_
  Advanced applications of HoloMap for experienced users. (Coming soon!)

* `Elements: <http://ioam.github.com/media/holoviews/Elements.html>`_
  Examples of all the major HoloViews elements and their usage.

* `Containers: <http://ioam.github.com/media/holoviews/Containers.html>`_
  Examples of all the HoloViews container types and their usage.

For more detailed information on particular components, please refer
to the `Reference Manual: <../Reference_Manual>`_ or the online help
(e.g. type ``Image?`` in an IPython Notebook cell).
