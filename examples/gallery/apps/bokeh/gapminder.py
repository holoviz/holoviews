...
gapminder_df = pd.merge(
    pd.merge(pd.merge(fertility, population), life_expectancy), gapminder.regions, on="Country"
)
...
text = gapminder_ds.clone(
    {yr: hv.Text(1.2, 25, str(int(yr)), fontsize=30) for yr in gapminder_ds.keys()}
)
...
pn.Column(hvplot.state, pn.Row(slider, button), sizing_mode="stretch_both").servable(
    "Gapminder Demo"
)
